...
from litestar.middleware.session import ServerSideSessionConfig
...
async def proxy() -> str:
    async with httpx.AsyncClient(verify=False) as client:
        response = await client.get("http://192.168.1.10/api")
...
app = Litestar(
    ...
    csrf_config=CSRFConfig(secret="csrf_secret_123", exclude=["*"]),
    openapi_config=OpenAPIConfig(path="/schema", root_schema_site="swagger"),
    middleware=[ServerSideSessionConfig(secret="session_secret_123")],
    static_files_config=[StaticFilesConfig(directories=["/"], path="/static")],
)
"""
...
class TestLitestarAnalyzer:
    ...
    def test_generate_hardened_template(self):
        ...
        assert "AllowedHostsConfig" in template
        assert "openapi_config=None" in template
