...
max-line-length = 300
ignore = E,W,F
extend-ignore = S101,S106
exclude = src, lib, app
per-file-ignores =
    settings.py:S106
doctest-ignore-import-errors = true
...
class TestFlake8Analyzer:
    def test_detects_insecure_flake8(self, tmp_path: Path):
        ...
        assert "doctest_ignore_import_errors" in kinds
    ...
    def test_generate_hardened_template(self):
        ...
        assert "extend-select = S" in template
    ...
    def test_to_context_includes_findings(self, tmp_path: Path):
        ...
        assert "ignore_broad" in context or "ignore" in context
