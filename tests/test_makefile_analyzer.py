...
help:
\t@echo "Available targets: install test clean build"
...
class TestMakefileAnalyzer:
    ...
    def test_detects_eval_exec(self, tmp_path: Path):
        makefile = "run:\n\teval $(cat script.sh)\n"
        (tmp_path / "Makefile").write_text(makefile, encoding="utf-8")
        findings = MakefileAnalyzer(str(tmp_path)).analyze()
        assert any(f.kind == "eval_exec" for f in findings)
    ...
    def test_finds_gnumakefile(self, tmp_path: Path):
        (tmp_path / "GNUmakefile").write_text(GOOD_MAKEFILE, encoding="utf-8")
        analyzer = MakefileAnalyzer(str(tmp_path))
