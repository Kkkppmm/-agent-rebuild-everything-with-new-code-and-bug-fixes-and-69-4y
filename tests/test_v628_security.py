...
class TestInsecureLoggingSettingsAnalyzer:
    def test_clean_logging_settings(self, tmp_path: Path):
        (tmp_path / "settings.py").write_text(
            ...
            "    'version': 1,\n"
            "    'handlers': {\n"
            "        'file': {'class': 'logging.handlers.RotatingFileHandler', 'filename': '/var/log/app.log'},\n"
            "    },\n"
            "    'loggers': {\n"
    ...
    def test_detects_log_level_assignment(self, tmp_path: Path):
    ...
    def test_console_ok_in_test_file(self, tmp_path: Path):
        ...
        findings = InsecureLoggingSettingsAnalyzer(str(tmp_path)).analyze()
        assert not any(f.pattern == "console_handler_in_production" for f in findings)
