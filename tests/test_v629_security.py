...
class TestInsecureCorsSettingsAnalyzer:
    ...
    def test_detects_credentials_with_allow_all(self, tmp_path: Path):
        (tmp_path / "settings.py").write_text(
            "CORS_ALLOW_ALL_ORIGINS = True\n"
    ...
    def test_detects_legacy_origin_allow_all(self, tmp_path: Path):
        (tmp_path / "settings.py").write_text(
    ...
    def test_cors_ok_in_test_file(self, tmp_path: Path):
        (tmp_path / "test_cors.py").write_text(
            "CORS_ALLOW_ALL_ORIGINS = True\n",
            encoding="utf-8",
        )
        findings = InsecureCorsSettingsAnalyzer(str(tmp_path)).analyze()
        assert not any(f.pattern == "cors_allow_all_origins" for f in findings)
