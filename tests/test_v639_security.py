...
class TestInsecureCspSettingsAnalyzer:
    ...
    def test_detects_unsafe_inline(self, tmp_path: Path):
        ...
        findings = InsecureCspSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "csp_unsafe_inline" for f in findings)
    ...
    def test_detects_unsafe_eval(self, tmp_path: Path):
        ...
        findings = InsecureCspSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "csp_unsafe_eval" for f in findings)
    ...
    def test_detects_wildcard_source(self, tmp_path: Path):
        ...
        findings = InsecureCspSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "csp_wildcard_source" for f in findings)

    def test_detects_csp_disabled(self, tmp_path: Path):
        (tmp_path / "settings.py").write_text(
            "CSP_ENABLED = False\n",
            encoding="utf-8",
        )
    ...
    def test_detects_report_only(self, tmp_path: Path):
        (tmp_path / "config.py").write_text(
            "CSP_REPORT_ONLY = True\n",
            encoding="utf-8",
        )
        findings = InsecureCspSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "csp_report_only" for f in findings)

    def test_detects_data_script_src(self, tmp_path: Path):
        (tmp_path / "security.py").write_text(
            "CSP_SCRIPT_SRC = (\"'self'\", 'data:')\n",
            encoding="utf-8",
        )
        findings = InsecureCspSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "csp_data_script_src" for f in findings)
