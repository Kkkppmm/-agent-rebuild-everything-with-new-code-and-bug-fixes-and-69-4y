...
class TestDevAI:
    ...
    def test_run_inline(self):
        ...
        plan = ai.dry_run_inline(
            '{"name": "inline", "tasks": [{"name": "security", "action": "security"}]}',
            {"code": "pass"},
        )
        assert plan[0].action == "security"

    def test_from_env_mock(self, monkeypatch):
        monkeypatch.setenv("DEVAI_PROVIDER", "mock")
        ai = DevAI.from_env()
        assert isinstance(ai, DevAI)
        assert ai.review("def foo(): pass")
    ...
    def test_security_analyzers(self, tmp_path):
        ...
        assert ai.command_injection(str(tmp_path)).health_score() >= 0
