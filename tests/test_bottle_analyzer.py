...
from bottle import Bottle, run, request, response, redirect, static_file, template
...
def serve_static(filename):
    return static_file(filename, root=request.query.root)


@app.route("/render")
def render_page():
    return template(request.query.page)
...
class TestBottleAnalyzer:
    def test_detects_insecure_bottle_app(self, tmp_path: Path):
        ...
        assert "static_file_user_input" in kinds
