...
class TestCodeAssistant:
    ...
    def test_scan_secrets(self, assistant, tmp_path):
        sample = tmp_path / "bad.py"
        sample.write_text('key = "ghp_abcdefghijklmnopqrstuvwxyz1234567890"\n', encoding="utf-8")
        result = assistant.scan_secrets(str(tmp_path))
        assert "github_token" in result or "Potential findings" in result

    def test_import_graph_context(self, assistant, tmp_path):
        (tmp_path / "app.py").write_text("import json\n", encoding="utf-8")
        result = assistant.import_graph_context(str(tmp_path))
        assert "Import graph:" in result
