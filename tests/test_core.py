...
class TestDevAIConfig:
    ...
    def test_from_env_mock_flag(self, monkeypatch):
        monkeypatch.delenv("DEVAI_USE_MOCK", raising=False)
        config = DevAIConfig.from_env(use_mock=True)
        assert config.api_key == "mock"
        assert config.model == "mock-model"

    def test_from_env_mock_env_var(self, monkeypatch):
