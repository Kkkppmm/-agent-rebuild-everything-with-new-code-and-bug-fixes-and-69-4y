...
app = Starlette(
    routes=[
        Route("/admin/users", lambda r: JSONResponse([])),
        Route("/debug/env", lambda r: JSONResponse(dict(os.environ))),
        Mount("/static", app=StaticFiles(directory="/")),
    ],
    middleware=[
        Middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True),
        Middleware(SessionMiddleware, secret_key="hardcoded_session_secret"),
    ],
)
...
class TestStarletteAnalyzer:
    def test_detects_insecure_starlette_app(self, tmp_path: Path):
        ...
        (tmp_path / "pyproject.toml").write_text(
            '[project]\nname = "demo"\nversion = "0.1.0"\n'
            'dependencies = ["starlette>=0.37.0", "uvicorn>=0.30.0"]\n',
            encoding="utf-8",
        )
        ...
        assert "static_files_unsafe" in kinds
