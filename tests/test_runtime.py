...
class TestDevRuntime:
    ...
    def test_run_inline_json(self):
        runtime = DevRuntime.create(use_mock=True)
        results = runtime.run_inline(
            '{"name": "inline", "tasks": [{"name": "review", "action": "review"}]}',
            {"code": "x = 1"},
        )
        assert len(results) == 1
        assert results[0].action == "review"
    ...
    def test_run_inline_dict(self):
        runtime = DevRuntime.create(use_mock=True)
        results = runtime.run_inline(
            {"name": "dict", "tasks": [{"name": "explain", "action": "explain"}]},
            {"code": "x = 1"},
        )
        assert len(results) == 1
    ...
    @pytest.mark.asyncio
    async def test_arun_inline(self):
        runtime = DevRuntime.create(use_mock=True)
        results = await runtime.arun_inline(
            {"name": "async-inline", "tasks": [{"name": "review", "action": "review"}]},
            {"code": "def foo(): pass"},
        )
        assert len(results) == 1
