...
class TestDevRuntime:
    ...
    def test_git_and_trace(self):
        runtime = DevRuntime.create(use_mock=True)
        git = runtime.git(".")
        assert git.is_repo()
        trace = runtime.trace("test")
        with trace.span("run"):
            runtime.run("pre-commit", {"code": "pass"})
        assert trace.spans

    def test_from_project_mock(self):
        runtime = DevRuntime.from_project(use_mock=True)
        assert runtime.assistant is not None
