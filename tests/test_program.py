...
"""
    ...
    def test_from_inline_dict(self):
        assistant = CodeAssistant(client=MockLLMClient())
        program = DevProgram.from_inline(
            {"name": "dict-program", "tasks": [{"name": "explain", "action": "explain"}]},
            assistant,
        )
        assert program.name == "dict-program"
        assert program.tasks[0].action == "explain"
