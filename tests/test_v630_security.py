...
class TestInsecureStorageSettingsAnalyzer:
    ...
    def test_detects_filesystem_storage(self, tmp_path: Path):
        ...
        findings = InsecureStorageSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "local_file_storage_in_production" for f in findings)
        assert any(f.severity == "high" for f in findings)

    def test_detects_storages_filesystem_backend(self, tmp_path: Path):
    ...
    def test_detects_unsigned_s3_urls(self, tmp_path: Path):
        (tmp_path / "config.py").write_text(
            "AWS_QUERYSTRING_AUTH = False\n",
            encoding="utf-8",
        )
        findings = InsecureStorageSettingsAnalyzer(str(tmp_path)).analyze()
        assert any(f.pattern == "s3_unsigned_urls" for f in findings)
    ...
    def test_filesystem_ok_in_test_file(self, tmp_path: Path):
        ...
        findings = InsecureStorageSettingsAnalyzer(str(tmp_path)).analyze()
        assert not any(f.pattern == "local_file_storage_in_production" for f in findings)
