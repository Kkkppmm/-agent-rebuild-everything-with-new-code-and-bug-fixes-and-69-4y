...
CSRF_TRUSTED_ORIGINS_WILDCARD_PATTERN = re.compile(
...
CSRF_DISABLED_PATTERN = re.compile(
    r"csrf\s*:\s*(?:false|\{[^}]*checkOrigin\s*:\s*false)",
    re.IGNORECASE | re.DOTALL,
)
PATHS_RELATIVE_PATTERN = re.compile(
    r"paths\s*:\s*\{[^}]*(?:base|assets)\s*:\s*['\"]\.\./",
    re.IGNORECASE | re.DOTALL,
)
...
SERVICE_WORKER_ENABLED_PATTERN = re.compile(
...
def _looks_like_sveltekit_project(root: Path) -> bool:
    ...
    if pkg.is_file():
        try:
            ...
            if isinstance(deps, dict) and ("@sveltejs/kit" in deps or "svelte" in deps):
                return True
...
class SvelteKitAnalyzer:
    ...
    exposed dev servers, internal proxy targets, and permissive filesystem access.
    """
    ...
    def _scan_line(
        ...
        for section in ("kit", "adapter", "vite", "preprocess", "csrf"):
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (PRERENDER_ORIGIN_HTTP_PATTERN, "prerender_origin_http", "high",
             "prerender origin uses HTTP — canonical URLs should use HTTPS"),
            ...
            (CSRF_DISABLED_PATTERN, "csrf_disabled", "high",
             "CSRF protection disabled — form actions vulnerable to cross-site attacks"),
            (HOST_EXPOSED_PATTERN, "host_exposed", "medium",
             "dev server host exposed to all interfaces — restrict to localhost in dev"),
            (CSRF_CHECK_ORIGIN_FALSE_PATTERN, "check_origin_disabled", "medium",
             "kit.csrf.checkOrigin disabled — CSRF protection weakened"),
            (CSRF_TRUSTED_ORIGINS_WILDCARD_PATTERN, "trusted_origins_wildcard", "medium",
             "kit.csrf.trustedOrigins includes * — any origin may submit forms"),
            ...
            (PATHS_RELATIVE_PATTERN, "paths_relative", "medium",
             "kit.paths uses relative parent paths — may expose files outside project root"),
            (SERVICE_WORKER_ENABLED_PATTERN, "service_worker_enabled", "low",
             "service worker registration enabled — verify cache and update policies"),
        ]
