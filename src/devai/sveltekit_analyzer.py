...
CSP_OFF_PATTERN = re.compile(
    r"(?:csp|contentSecurityPolicy)\s*:\s*(?:false|['\"]off['\"]|{\s*mode\s*:\s*['\"]off['\"])",
    re.IGNORECASE,
)
...
PRERENDER_SECRET_PATTERN = re.compile(
    r"prerender\s*:\s*\{[^}]*(?:entries|handleHttpError)\s*:\s*[^}]*(?:secret|token|key)",
    re.IGNORECASE | re.DOTALL,
)
VERSION_POLLUTION_PATTERN = re.compile(
    r"version\s*:\s*\{[^}]*poll\s*:\s*(?:\d+|['\"]\d+['\"])",
    re.IGNORECASE | re.DOTALL,
)
EMBEDDED_ENABLED_PATTERN = re.compile(
    r"embedded\s*:\s*(?:true|['\"]true['\"])",
    re.IGNORECASE,
)
...
class SvelteKitAnalyzer:
    ...
    exposed dev servers, disabled CSP, internal proxy targets, adapter
    credential leaks, and permissive Vite filesystem access.
    """
    ...
    def _scan_line(
        ...
        for section in ("kit", "adapter", "vite", "csrf", "csp", "prerender", "paths"):
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (CSRF_CHECK_ORIGIN_FALSE_PATTERN, "csrf_disabled", "high",
             "CSRF checkOrigin disabled — cross-site request forgery protection weakened"),
            (CSRF_TRUSTED_ORIGINS_WILDCARD_PATTERN, "csrf_trusted_wildcard", "high",
             "CSRF trustedOrigins includes * — any origin may bypass CSRF checks"),
            ...
            (PRERENDER_SECRET_PATTERN, "prerender_secret", "medium",
             "prerender config may leak secrets — verify entries and handleHttpError"),
            (VERSION_POLLUTION_PATTERN, "version_pollution", "low",
             "version.poll enabled — may leak deployment metadata"),
            (EMBEDDED_ENABLED_PATTERN, "embedded_enabled", "low",
             "embedded mode enabled — verify iframe embedding policy"),
        ]
