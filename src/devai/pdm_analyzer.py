...
PDM_STANDALONE_NAMES = (".pdm.toml",)

PDM_MARKER_PATTERN = re.compile(
    r"(?:^\[tool\.pdm\]|^\[tool\.pdm\.|pdm-backend|pdm\s*=\s*\{)",
...
def _is_pdm_file(path: Path) -> bool:
    ...
    name = path.name
    if name in PDM_STANDALONE_NAMES:
        return True
...
class PdmAnalyzer:
    ...
    def _analyze_file(self, path: Path) -> tuple[list[PdmFinding], PdmInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    PdmFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Pdm config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    PdmFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Pdm config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:dependencies|dev-dependencies|optional-dependencies|groups)", stripped, re.IGNORECASE)
        ...
        lock_path = path.parent / "pdm.lock"
        if path.name in ("pyproject.toml", "Pipfile") and not lock_path.exists():
            findings.append(
                PdmFinding(
                    kind="missing_lockfile",
                    severity="low",
                    message="pdm.lock missing — commit lockfile for reproducible installs",
                    path=rel,
                    lineno=1,
                    line="",
                )
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# .pdm.toml — hardened PDM defaults
[pypi]
# Use HTTPS indexes; store credentials via PDM_PYPI_USERNAME/PASSWORD env vars
