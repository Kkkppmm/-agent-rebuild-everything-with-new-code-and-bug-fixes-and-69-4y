    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden pnpm — enable trust-policy=strict, keep strict-ssl=true, store tokens via NPM_TOKEN env vars, avoid shamefully-hoist, pin catalog versions, and review .pnpmfile.cjs hooks for curl|sh patterns"
                )
            elif cat.name == "bun" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Bun — use HTTPS registries, store tokens via env vars, pin git dependencies to commits, disable global install.scripts, keep TLS verification enabled, and commit bun.lock for reproducible installs"
                )
            elif cat.name == "deno" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Deno — use least-privilege permissions instead of --allow-all, pin npm:/jsr: imports to versions, use HTTPS import URLs, store secrets via Deno.env, and review task scripts for dangerous commands"
                )
