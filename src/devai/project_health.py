    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Kubernetes manifests — avoid privileged pods, host namespaces, and secrets in env"
                )
            elif cat.name == "k8s" and cat.score < 70 and cat.details.get("manifest_files", 0) > 0:
                recs.append(
                    "Review K8s findings — set securityContext, pin images, and add resource limits"
                )
