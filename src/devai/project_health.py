    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid curl-pipe-to-shell, dangerous rm -rf, and hardcoded secrets"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — pin dependencies and avoid sudo/eval in recipes"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — disable privileged mode, host namespaces, and root execution"
                )
            elif cat.name == "kubernetes" and cat.score < 70 and cat.details.get("manifest_files", 0) > 0:
                recs.append(
                    "Review K8s findings — pin images, set resource limits, and use secretKeyRef for secrets"
                )
        return recs
