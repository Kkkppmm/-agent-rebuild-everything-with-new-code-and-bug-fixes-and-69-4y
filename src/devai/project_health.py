    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden maturin — pin git dependencies in Cargo.toml to tags/commits, store PyPI tokens via MATURIN_PASSWORD from CI secrets, avoid curl-pipe-to-shell in pre-build hooks, use pyo3 bindings with extension-module, keep auditwheel enabled, and avoid including sensitive paths in wheel data"
                )
