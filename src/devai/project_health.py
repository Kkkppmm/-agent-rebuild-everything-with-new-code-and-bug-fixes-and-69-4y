    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — remove dangerous rm patterns, secrets, and curl-pipe-to-shell"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — avoid sudo, pin Docker images, and declare .PHONY targets"
                )
        return recs
