...
class ProjectHealth:
    ...
    def _score_precommit(self, analyzer: PrecommitAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.config_files} config file(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden pre-commit config — pin hook revs to tags/SHAs and avoid secrets in args"
                )
            elif cat.name == "precommit" and cat.score < 70 and cat.details.get("config_files", 0) > 0:
                recs.append(
                    "Review pre-commit findings — use HTTPS repos and audit local hook entries"
                )
        return recs
