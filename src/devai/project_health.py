    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden CircleCI config — pin orbs to versions and avoid secrets in environment"
                )
            elif cat.name == "bitbucket_pipelines" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Bitbucket Pipelines — pin pipes and use secured variables for secrets"
                )
            elif cat.name == "gitlab_ci" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden GitLab CI — pin images, mask variables, and avoid privileged services"
                )
            elif cat.name == "jenkinsfile" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Jenkinsfile — use credentials binding instead of inline secrets"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — run as non-root, pin images, and set resource limits"
                )
        return recs
