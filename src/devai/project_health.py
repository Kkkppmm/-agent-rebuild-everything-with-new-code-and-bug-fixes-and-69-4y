...
class ProjectHealth:
    ...
    def _score_lefthook(self, analyzer: LefthookAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No lefthook configs found", {"configs": 0, "findings": 0}
        summary = (
            f"{stats.configs} lefthook config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden lefthook — store secrets via CI or gitignored env files, avoid curl|sh in hook commands, pin remote extends and remote.git.ref to commit SHAs, use HTTPS remote URLs, and review pre-commit/pre-push scripts for sudo and chmod 777"
                )
