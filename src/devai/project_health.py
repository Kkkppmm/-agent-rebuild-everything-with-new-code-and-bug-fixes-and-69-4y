...
class ProjectHealth:
    ...
    def _score_aws_codepipeline(self, analyzer: AWSCodePipelineAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.pipelines} CodePipeline config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "pipelines": stats.pipelines,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden AWS CodePipeline — enable KMS encryption on artifact stores, use Secrets Manager for tokens, apply least-privilege IAM, and add approval stages"
                )
        return recs
