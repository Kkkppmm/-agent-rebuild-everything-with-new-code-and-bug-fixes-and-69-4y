...
class ProjectHealth:
    ...
    def _score_circleci(self, analyzer: CircleCIAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No CircleCI configs found", {"configs": 0, "findings": 0}
        summary = (
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden CircleCI configs — pin orbs to release versions and avoid secrets in environment"
                )
            elif cat.name == "circleci" and cat.score < 70 and cat.details.get("configs", 0) > 0:
                recs.append(
                    "Review CircleCI findings — pin Docker images and run jobs as non-root"
                )
            elif cat.name == "gitlab_ci" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden GitLab CI configs — avoid privileged runners and secrets in variables"
                )
            elif cat.name == "gitlab_ci" and cat.score < 70 and cat.details.get("configs", 0) > 0:
                recs.append(
                    "Review GitLab CI findings — pin images and restrict artifact collection"
                )
            elif cat.name == "travis_ci" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Travis CI configs — use encrypted variables and avoid curl-pipe-to-shell"
                )
