...
class ProjectHealth:
    ...
    def _score_vite(self, analyzer: ViteAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.config_files == 0:
            return 100.0, "No Vite configs found", {"config_files": 0, "findings": 0}
        summary = (
            f"{stats.config_files} Vite config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "config_files": stats.config_files,
    ...
    def _score_tsconfig(self, analyzer: TsconfigAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.config_files == 0:
            return 100.0, "No TypeScript configs found", {"config_files": 0, "findings": 0}
        summary = (
