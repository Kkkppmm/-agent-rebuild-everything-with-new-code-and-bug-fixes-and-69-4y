...
class ProjectHealth:
    ...
    def _score_pelican(self, analyzer: PelicanAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.config_files == 0:
    ...
    def _score_eleventy(self, analyzer: EleventyAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.config_files == 0:
            return 100.0, "No Eleventy configs found", {"config_files": 0, "findings": 0}
        summary = (
            f"{stats.config_files} Eleventy config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "config_files": stats.config_files,
    ...
    def _score_starlight(self, analyzer: StarlightAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.config_files == 0:
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        eleventy = EleventyAnalyzer(root_str)
        score, summary, details = self._score_eleventy(eleventy)
        categories.append(HealthCategory("eleventy", score, summary, details))
