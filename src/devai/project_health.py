...
class ProjectHealth:
    ...
    def _score_infra_analyzer(
        ...
        *,
        ...
        if count == 0:
            return 100.0, empty_label, {count_attr: 0, "findings": 0}
        summary = (
            f"{count} file(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            count_attr: count,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid dangerous rm, curl-pipe-to-shell, and secrets in variables"
                )
                ...
                recs.append(
                    "Harden Terraform — restrict security groups, enable encryption, and remove hardcoded secrets"
                )
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        score, summary, details = self._score_infra_analyzer(
            makefile, name="makefile", count_attr="makefiles", empty_label="No Makefiles found"
        )
