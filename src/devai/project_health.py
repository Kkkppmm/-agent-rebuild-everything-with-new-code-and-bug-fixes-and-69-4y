...
class ProjectHealth:
    ...
    def _score_deno(self, analyzer: DenoAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.configs} Deno project(s), "
            f"{stats.findings} finding(s) ({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _score_vitest(self, analyzer: VitestAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.config_files == 0:
            return 100.0, "No Vitest configs found", {"config_files": 0, "findings": 0}
        summary = (
            f"{stats.config_files} Vitest config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "config_files": stats.config_files,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Bun — use HTTPS registries, keep TLS verification enabled, pin exact versions, set ignoreScripts=true, avoid trustedDependencies wildcards, and store tokens via env vars"
                )
            elif cat.name == "deno" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Deno — use least-privilege --allow-* flags, avoid --allow-all, pin import map URLs, store secrets via env vars, and review task scripts for eval or curl|sh"
                )
