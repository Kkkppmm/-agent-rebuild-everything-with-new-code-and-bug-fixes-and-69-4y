    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid curl-pipe-to-shell, dangerous rm -rf, and force-push targets"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — declare .PHONY targets and avoid hardcoded secrets"
                )
        return recs
