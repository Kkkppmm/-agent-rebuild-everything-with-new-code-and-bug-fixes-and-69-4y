    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid dangerous rm patterns, curl-pipe-to-shell, and hardcoded secrets"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — quote shell variables and avoid sudo in recipes"
                )
                ...
                recs.append(
                    "Review K8s findings — set securityContext, pin images, and use secretKeyRef"
                )
        return recs
