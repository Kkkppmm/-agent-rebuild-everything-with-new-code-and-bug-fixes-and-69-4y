    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Jenkins pipelines — use credentials store, pin libraries, and avoid privileged agents"
                )
            elif cat.name == "jenkins" and cat.score < 70 and cat.details.get("jenkinsfiles", 0) > 0:
                recs.append(
                    "Review Jenkinsfile findings — restrict agent labels and avoid curl-pipe-to-shell"
                )
        return recs
