    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden pre-commit config — pin hook revs to tags or SHAs and avoid local hooks"
                )
            elif cat.name == "precommit" and cat.score < 70 and cat.details.get("config_files", 0) > 0:
                recs.append(
                    "Review pre-commit findings — add default_language_version and pin all hook versions"
                )
            elif cat.name == "makefile" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Makefiles — remove destructive rm commands, secrets, and curl-pipe-to-shell"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — add .PHONY declarations and avoid sudo in build targets"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — disable privileged mode, host namespaces, and secrets in env"
                )
            elif cat.name == "kubernetes" and cat.score < 70 and cat.details.get("manifests", 0) > 0:
                recs.append(
                    "Review K8s findings — pin image tags, set resource limits, and run as non-root"
                )
        return recs
