...
from devai.k8s_analyzer import K8sManifestAnalyzer
...
class ProjectHealth:
    ...
    def _score_k8s(self, analyzer: K8sManifestAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Kubernetes manifests — avoid privileged pods, host namespaces, and secrets in env literals"
                )
            elif cat.name == "k8s" and cat.score < 70 and cat.details.get("manifest_files", 0) > 0:
                recs.append(
                    "Review Kubernetes findings — set runAsNonRoot, resource limits, and avoid hostPath mounts"
                )
        return recs
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        k8s = K8sManifestAnalyzer(root_str)
