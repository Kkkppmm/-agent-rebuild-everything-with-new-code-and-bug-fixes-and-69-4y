    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Docker Compose — avoid privileged mode, host networking, and secrets in env"
                )
            elif cat.name == "workflow" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Review GitHub Actions workflows — pin actions to SHAs and restrict permissions"
                )
            elif cat.name == "precommit" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Pin pre-commit hook revs to tags or commits instead of main/master"
                )
            elif cat.name == "makefile" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Review Makefile targets — remove dangerous rm commands and hardcoded secrets"
                )
        return recs
