    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden CircleCI — pin orbs to semver, use contexts for secrets, and avoid curl-pipe-to-shell"
                )
            elif cat.name == "bitbucket_pipelines" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Bitbucket Pipelines — use secured variables, pin images, and block on security steps"
                )
        return recs
