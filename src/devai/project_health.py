...
class ProjectHealth:
    ...
    def _score_pep723(self, analyzer: Pep723Analyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.script_blocks == 0:
            return 100.0, "No PEP 723 script blocks found", {"script_blocks": 0, "findings": 0}
        summary = (
            f"{stats.script_blocks} PEP 723 block(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "script_blocks": stats.script_blocks,
            "findings": stats.findings,
            "high_severity": stats.high_severity,
            "medium_severity": stats.medium_severity,
            "low_severity": stats.low_severity,
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        pep723 = Pep723Analyzer(root_str)
        score, summary, details = self._score_pep723(pep723)
        categories.append(HealthCategory("pep723", score, summary, details))
