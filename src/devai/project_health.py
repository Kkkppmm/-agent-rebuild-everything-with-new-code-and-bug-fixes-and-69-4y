...
class ProjectHealth:
    ...
    def _score_terraform(self, analyzer: TerraformAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.files == 0:
            return 100.0, "No Terraform files found", {"files": 0, "findings": 0}
        summary = (
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid curl-pipe-to-shell, sudo, and dangerous rm targets"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — disable privileged mode and pin image tags"
                )
