    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — avoid curl-pipe-to-shell, sudo, and secrets in variables"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — use .PHONY and safe cleanup targets"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — disable privileged mode and host namespaces"
                )
            elif cat.name == "kubernetes" and cat.score < 70 and cat.details.get("manifests", 0) > 0:
                recs.append(
                    "Review K8s findings — pin images, set resource limits, and run as non-root"
                )
        return recs
