...
class ProjectHealth:
    ...
    def _score_flask(self, analyzer: FlaskAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No Flask apps found", {"configs": 0, "findings": 0}
        summary = (
            f"{stats.configs} Flask app(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _score_fastapi(self, analyzer: FastAPIAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No FastAPI apps found", {"configs": 0, "findings": 0}
        summary = (
            f"{stats.configs} FastAPI app(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _score_django(self, analyzer: DjangoAnalyzer) -> tuple[float, str, dict]:
        ...
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No Django settings found", {"configs": 0, "findings": 0}
        summary = (
            f"{stats.configs} Django settings file(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
