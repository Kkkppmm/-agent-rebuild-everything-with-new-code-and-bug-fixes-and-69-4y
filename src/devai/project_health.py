...
class ProjectHealth:
    ...
    def _score_mise(self, analyzer: MiseAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.configs} mise project(s), "
            f"{stats.findings} finding(s) ({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _score_pnpm(self, analyzer: PnpmAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.configs} pnpm project(s), "
            f"{stats.findings} finding(s) ({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden pnpm — use HTTPS registries, store tokens in env vars, enable strict-peer-dependencies, use ignore-scripts in CI, avoid shamefully-hoist unless required, and pin git dependencies to commits"
                )
            elif cat.name == "bun" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Bun — use HTTPS registries, keep TLS verification enabled, restrict trustedDependencies, enable ignoreScripts in CI, and pin git dependencies to commits"
                )
