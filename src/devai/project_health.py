...
class ProjectHealth:
    ...
    def _score_openapi(self, analyzer: OpenAPIAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.specs == 0:
            return 100.0, "No OpenAPI specs found", {"specs": 0, "findings": 0}
        summary = (
            f"{stats.specs} OpenAPI spec(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "specs": stats.specs,
    ...
    def _score_buf(self, analyzer: BufAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.configs == 0:
            return 100.0, "No Buf configs found", {"configs": 0, "findings": 0}
        summary = (
            f"{stats.configs} Buf config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "configs": stats.configs,
    ...
    def _score_hugo(self, analyzer: HugoAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.config_files == 0:
    ...
    def _score_vitepress(self, analyzer: VitePressAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.config_files == 0:
            return 100.0, "No VitePress configs found", {"config_files": 0, "findings": 0}
        summary = (
            f"{stats.config_files} VitePress config(s), {stats.findings} finding(s) "
            f"({stats.high_severity} high)"
        )
        return score, summary, {
            "config_files": stats.config_files,
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            ...
            elif cat.name == "openapi" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden OpenAPI specs — use HTTPS server URLs, remove credentials from specs, require auth on admin/debug paths, avoid API keys in query strings, and prefer authorization code flow over implicit OAuth"
                )
            elif cat.name == "buf" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Buf configs — enable DEFAULT lint and FILE breaking rules, pin remote plugin versions, use HTTPS registries, and avoid curl|sh in plugin options"
                )
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        openapi = OpenAPIAnalyzer(root_str)
        score, summary, details = self._score_openapi(openapi)
        categories.append(HealthCategory("openapi", score, summary, details))

        buf = BufAnalyzer(root_str)
        score, summary, details = self._score_buf(buf)
        categories.append(HealthCategory("buf", score, summary, details))
        ...
        categories.append(HealthCategory("vitepress", score, summary, details))
