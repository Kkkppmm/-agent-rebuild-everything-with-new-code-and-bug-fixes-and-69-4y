    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Go Task — avoid curl|sh in task commands, store secrets via env vars or CI secret stores, use dotenv only for gitignored files, and review sudo/destructive shell patterns in recipes"
                )
            elif cat.name == "justfile" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden just — avoid curl|sh in recipes, store secrets via env_var() or CI secrets, use dotenv-load only with gitignored .env files, and review sudo/destructive shell patterns"
                )
