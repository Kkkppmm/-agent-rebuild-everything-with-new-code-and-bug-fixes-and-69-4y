    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            if cat.name == "typing" and cat.score < 80:
                ...
                recs.append(
                    "Harden Makefiles — remove sudo, avoid curl-pipe-to-shell, and eliminate destructive rm"
                )
            elif cat.name == "makefile" and cat.score < 70 and cat.details.get("makefiles", 0) > 0:
                recs.append(
                    "Review Makefile findings — add .PHONY for common targets and pin git dependencies"
                )
        return recs
