...
class ProjectHealth:
    ...
    def _score_workflow(self, analyzer: WorkflowAnalyzer) -> tuple[float, str, dict]:
        analyzer.analyze()
        score = analyzer.health_score()
        stats = analyzer.stats
        if stats.workflows == 0:
            return 100.0, "No workflows found", {"workflows": 0, "findings": 0}
        summary = (
    ...
    def _score_compose(self, analyzer: ComposeAnalyzer) -> tuple[float, str, dict]:
        ...
        if stats.compose_files == 0:
            return 100.0, "No compose files found", {"compose_files": 0, "findings": 0}
        summary = (
    ...
    def _score_precommit(self, analyzer: PrecommitAnalyzer) -> tuple[float, str, dict]:
        ...
        summary = (
            f"{stats.config_files} config(s), {stats.hooks} hook(s), "
    ...
    def _build_recommendations(self, categories: list[HealthCategory]) -> list[str]:
        ...
        for cat in categories:
            ...
            elif cat.name == "workflow" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden CI workflows — pin actions, avoid pull_request_target with untrusted checkout"
                )
            elif cat.name == "compose" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Docker Compose — disable privileged mode and avoid docker.sock mounts"
                )
            elif cat.name == "precommit" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Review pre-commit hooks — remove dangerous shell commands from hook entries"
                )
            elif cat.name == "precommit" and cat.score < 70 and cat.details.get("config_files", 0) > 0:
                recs.append(
                    "Pin pre-commit hook revisions to specific tags or commit SHAs"
                )
            elif cat.name == "kubernetes" and cat.details.get("high_severity", 0) > 0:
                recs.append(
                    "Harden Kubernetes manifests — disable privileged mode and avoid host namespaces"
                )
            elif cat.name == "kubernetes" and cat.score < 70 and cat.details.get("manifest_files", 0) > 0:
                recs.append(
                    "Review Kubernetes findings — pin images, run as non-root, and use Secrets for credentials"
                )
        return recs
    ...
    def analyze(self) -> ProjectHealthReport:
        ...
        workflow = WorkflowAnalyzer(root_str)
        score, summary, details = self._score_workflow(workflow)
        categories.append(HealthCategory("workflow", score, summary, details))
