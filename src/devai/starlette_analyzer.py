...
STARLETTE_ROUTE_PATTERN = re.compile(
    r"(?:Route\s*\(|Mount\s*\(|@(?:app|router)\.(?:route|get|post|put|delete|patch)\b)",
    re.IGNORECASE,
)
...
DANGEROUS_ROUTE_PATTERN = re.compile(
    r"(?:Route|Mount)\s*\(\s*['\"](?:/)?(?:admin|debug|internal)",
    re.IGNORECASE,
)
...
class StarletteAnalyzer:
    ...
    Scans Starlette entry files, routes, and middleware for hardcoded secrets,
    open CORS, unsafe StaticFiles mounts, SSRF targets, and unprotected admin routes.
    """
    ...
    def _scan_line(
        ...
        if "CORSMiddleware" in stripped or "allow_origins" in stripped:
            info.has_cors = True
        if any(k in stripped for k in ("AuthenticationMiddleware", "requires", "HTTPBasicCredentials")):
            info.has_auth = True
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (SESSION_SECRET_HARDCODED_PATTERN, "session_secret_hardcoded", "high",
             "hardcoded SessionMiddleware secret_key — load from environment variables"),
            ...
            (HOST_EXPOSED_PATTERN, "host_exposed", "medium",
             "Starlette bound to 0.0.0.0 — ensure firewall and reverse proxy are configured"),
            (STATIC_FILES_UNSAFE_PATTERN, "static_files_unsafe", "high",
             "StaticFiles with absolute or parent directory path — path traversal risk"),
