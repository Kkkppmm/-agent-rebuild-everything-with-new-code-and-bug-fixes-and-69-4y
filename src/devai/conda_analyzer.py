"""CondaAnalyzer — audit Conda environment.yml and recipe meta.yaml for security and build hardening."""
...
CONDA_STANDALONE_NAMES = ("environment.yml", "environment.yaml", "meta.yaml")

CONDA_MARKER_PATTERN = re.compile(
    r"(?:^name:\s|^\s*-\s*conda-forge|channels:|dependencies:)",
...
def _is_conda_file(path: Path) -> bool:
    """Return True if the path looks like a Conda configuration file."""
    name = path.name
    if name in CONDA_STANDALONE_NAMES:
        return True
...
class CondaAnalyzer:
    """Audit Conda configuration for security issues.
    ...
    def configs(self) -> list[Path]:
        ...
        for path in sorted(self.root.rglob("*")):
            if path.is_file() and _is_conda_file(path):
                found.append(path)
        return found
    ...
    def _analyze_file(self, path: Path) -> tuple[list[CondaFinding], CondaInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    CondaFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Conda config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    CondaFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Conda config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:dependencies|run|host|build)", stripped, re.IGNORECASE)
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# environment.yml — hardened Conda defaults
name: myenv
channels:
  - conda-forge
dependencies:
