...
RYE_STANDALONE_NAMES = ("requirements.lock",)
...
def _is_rye_file(path: Path) -> bool:
    ...
    name = path.name
    if name in RYE_STANDALONE_NAMES:
        return True
...
class RyeAnalyzer:
    ...
    def _analyze_file(self, path: Path) -> tuple[list[RyeFinding], RyeInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    RyeFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Rye config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    RyeFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Rye config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:dependencies|dev-dependencies)", stripped, re.IGNORECASE)
        ...
        lock_path = path.parent / "rye.lock"
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# pyproject.toml [tool.rye] — hardened Rye defaults
[tool.rye]
# Use HTTPS sources; store credentials via environment variables
# Commit rye.lock and requirements.lock for reproducible installs
"""
    ...
    def summary(self) -> str:
        ...
        return (
            f"Rye configs: {stats.configs} config(s), "
