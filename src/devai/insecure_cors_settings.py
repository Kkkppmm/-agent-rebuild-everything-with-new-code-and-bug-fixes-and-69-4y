...
_ALLOW_CREDENTIALS_RE = re.compile(
    r"CORS_ALLOW_CREDENTIALS\s*=\s*True",
    re.IGNORECASE,
)
_WILDCARD_ORIGIN_LIST_RE = re.compile(
...
def _list_has_wildcard(node: ast.AST) -> bool:
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            ...
            if name in ("CORS_ALLOW_ALL_ORIGINS", "CORS_ORIGIN_ALLOW_ALL"):
                ...
                if _list_has_wildcard(node.value):
                    self._wildcard_origins = True
                    self._add(
                        node.lineno,
                        "cors_wildcard_origin",
                        "high",
                        "CORS allowed origins includes wildcard (*) — list explicit trusted domains",
                        setting=name,
                    )
    ...
    def finalize(self) -> None:
        if self._allow_credentials and (self._allow_all_origins or self._wildcard_origins):
            self._add(
                ...
                "critical",
                "CORS credentials enabled with permissive origins — attackers can steal sessions",
                setting="CORS_ALLOW_CREDENTIALS",
            )


class InsecureCorsSettingsAnalyzer:
    """Detect insecure CORS configuration in Django corsheaders and similar apps."""
    ...
    def _scan_source(
        ...
        has_allow_all = bool(_ALLOW_ALL_ORIGINS_RE.search(source))
        has_wildcard = bool(_WILDCARD_ORIGIN_LIST_RE.search(source))
        has_credentials = bool(_ALLOW_CREDENTIALS_RE.search(source))
        ...
        for lineno, line in enumerate(source.splitlines(), start=1):
            ...
            if _WILDCARD_ORIGIN_LIST_RE.search(line):
                findings.append(
                    InsecureCorsSettingsFinding(
                        path=rel,
                        lineno=lineno,
                        ...
                        severity="high",
                        message="CORS allowed origins includes wildcard (*) — list explicit trusted domains",
        ...
        if has_credentials and (has_allow_all or has_wildcard):
            findings.append(
                InsecureCorsSettingsFinding(
                    ...
                    severity="critical",
                    message="CORS credentials enabled with permissive origins — attackers can steal sessions",
                    setting="CORS_ALLOW_CREDENTIALS",
                )
            )
        return findings
