"""HeaderInjectionAnalyzer — detect HTTP response header injection risks."""
                    ...
                    if _is_user_controlled(node.value):
                        self.findings.append(
                            HeaderInjectionFinding(
                                ...
                                severity="high",
                                message="User-controlled value assigned to response header enables CRLF injection",
            ...
            if node.args and _is_user_controlled(node.args[0]):
                self.findings.append(
                    HeaderInjectionFinding(
                        path=self.path,
                        lineno=node.lineno,
                        pattern="set_header_user_name",
                        severity="high",
                        message="User-controlled header name enables response splitting",
                        function=self._current_function(),
                    )
                )
            if len(node.args) > 1 and _is_user_controlled(node.args[1]):
                self.findings.append(
                    HeaderInjectionFinding(
                        path=self.path,
                        lineno=node.lineno,
                        pattern="set_header_user_value",
                        severity="high",
                        message="User-controlled header value enables CRLF injection",
                        function=self._current_function(),
                    )
                )
            ...
            if node.args and _is_user_controlled(node.args[0]):
                self.findings.append(
                    HeaderInjectionFinding(
                        path=self.path,
                        lineno=node.lineno,
                        pattern="redirect_user_location",
                        severity="medium",
                        message="User-controlled redirect URL may enable open redirect or header injection",
                        function=self._current_function(),
                    )
                )
        self.generic_visit(node)
...
class HeaderInjectionAnalyzer:
    """Detect HTTP response header injection and CRLF risks in Python web apps."""
