...
SKIP_SOURCE_PATTERN = re.compile(
    r"(?:skip|extend_skip|skip_glob)\s*=\s*[^\n]*(?:^|[,\s\"'])(?:src|lib|app)(?:/|[\"',\s]|$)",
    re.IGNORECASE,
)
SKIP_GITIGNORE_FALSE_PATTERN = re.compile(r"skip_gitignore\s*=\s*false\b", re.IGNORECASE)
FORCE_SINGLE_LINE_PATTERN = re.compile(r"force_single_line\s*=\s*true\b", re.IGNORECASE)
BLACK_PROFILE_PATTERN = re.compile(r"profile\s*=\s*[\"']?black[\"']?\b", re.IGNORECASE)
LINE_LENGTH_NON_BLACK_PATTERN = re.compile(
    r"line_length\s*=\s*(?!88\b)\d+\b",
    re.IGNORECASE,
)
MULTI_LINE_OUTPUT_CONFLICT_PATTERN = re.compile(
    r"multi_line_output\s*=\s*(?:[0-5]|7|8)\b",
    re.IGNORECASE,
)
...
def _extract_profile_value(line: str) -> str | None:
    match = re.search(
        r"^profile\s*=\s*[\"']?([^\"'\s]+)[\"']?\s*$",
        line.strip(),
        re.IGNORECASE,
...
class IsortAnalyzer:
    ...
    skip patterns on source directories, honor_noqa=false, Black profile
  conflicts, hardcoded secrets, and overly broad skip settings.
    """
    ...
    def __init__(self, root: str) -> None:
        ...
        self._black_profile_active = False
    ...
    def _scan_line(
        ...
        profile = _extract_profile_value(stripped)
        if profile is not None:
            info.profile = profile
            if profile.lower() == "black":
                self._black_profile_active = True
        ...
        if HONOR_NOQA_FALSE_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    ...
                    severity="high",
                    message="honor_noqa=false ignores noqa directives — keep noqa honored for security suppressions",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if SKIP_SOURCE_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    kind="skip_source",
                    severity="medium",
                    message="skip/extend_skip skips source directories — narrow skip patterns",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if SKIP_GITIGNORE_FALSE_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    ...
                    severity="low",
                    message="skip_gitignore=false may sort ignored files — prefer default gitignore respect",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if FORCE_SINGLE_LINE_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    ...
                    severity="low",
                    message="force_single_line=true reduces import readability — prefer multi-line grouping",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if self._black_profile_active and LINE_LENGTH_NON_BLACK_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    kind="black_profile_conflict",
                    severity="medium",
                    message="profile=black with non-88 line_length conflicts with Black defaults — align line_length",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if self._black_profile_active and MULTI_LINE_OUTPUT_CONFLICT_PATTERN.search(line):
            findings.append(
                IsortFinding(
                    kind="black_profile_conflict",
                    severity="medium",
                    message="profile=black with custom multi_line_output conflicts with Black — remove override",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
