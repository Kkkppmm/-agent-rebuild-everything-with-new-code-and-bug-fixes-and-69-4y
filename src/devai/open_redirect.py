...
_USER_INPUT_RE = re.compile(
    r"(request|user|input|url|uri|redirect|next|return|continue|goto|target|"
    r"callback|dest|destination|link|href|location|forward|back|referer|referrer|"
...
_LOCATION_HEADERS = frozenset({"Location", "location"})
...
def _first_redirect_arg(node: ast.Call) -> ast.AST | None:
    ...
    for kw in node.keywords:
        if kw.arg in {"url", "location", "to", "href"}:
            return kw.value
    return None


def _is_open_redirect_call(node: ast.Call) -> tuple[str, str, str] | None:
    """Return (pattern, severity, message) for risky redirect patterns."""
    name = _call_name(node)
    parts = name.split(".")
    method = parts[-1] if parts else name

    if method in _REDIRECT_FUNCS:
        target = _first_redirect_arg(node)
        if target is not None and _is_dynamic_string(target):
            return (
    ...
    if method == "set_header" and len(node.args) >= 2:
        header = node.args[0]
        if isinstance(header, ast.Constant) and header.value in _LOCATION_HEADERS:
            if _is_dynamic_string(node.args[1]):
                return (
                    "dynamic_location_header",
                    "high",
                    "Location header set from user input — validate redirect destinations",
                )

    if method in {"get", "pop", "getlist"} and isinstance(node.func, ast.Attribute):
        attr = node.func.attr
        if attr in {"args", "GET", "POST", "form", "values", "query_params"}:
            parent = _call_name(node)
            if "redirect" in parent.lower() or _looks_like_user_input(node):
                return (
                    "request_param_redirect",
                    "medium",
                    "Request parameter may flow into redirect — validate before use",
                )
...
class _OpenRedirectVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        result = _is_open_redirect_call(node)
        if result:
...
class OpenRedirectAnalyzer:
    """Detect open redirect vulnerabilities in Python web applications.

    Flags Flask ``redirect()``, Django ``HttpResponseRedirect``, FastAPI
    ``RedirectResponse``, and dynamic Location headers built from user input.
    """
    ...
    def to_context(self, limit: int = 30) -> str:
        ...
        if not self._findings:
            lines.append("No user-controlled redirect patterns found.")
        else:
