...
CODE_SMELL = PromptTemplate(
    system="You identify code smells and suggest concrete refactorings.",
    template=(
        "Identify code smells in this code. For each smell, explain why it is problematic "
        "and how to fix it:\n\n```\n$code\n```\n\nContext: $context"
    ),
    input_variables=["code", "context"],
)
...
OPENAPI = PromptTemplate(
    system="You design and review OpenAPI specifications for REST APIs.",
    template=(
        "Review or generate an OpenAPI specification for this API code or description:\n\n"
