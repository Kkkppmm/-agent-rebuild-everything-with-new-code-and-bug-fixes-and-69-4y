...
CORS_CREDENTIALS_WILDCARD_PATTERN = re.compile(
    ...
    r"allow_credentials\s*=\s*True[\s\S]{0,120}['\"]\*['\"]",
    re.IGNORECASE,
)
...
class QuartAnalyzer:
    ...
    Scans Quart entry files, routes, and middleware for hardcoded secrets,
    ...
    def _scan_line(
        ...
        if "CORS" in stripped or "cors" in stripped.lower():
            info.has_cors = True
        if any(k in stripped for k in ("login_required", "auth_required", "QuartAuth")):
            info.has_auth = True
        if "Talisman" in stripped or "force_https" in stripped:
            info.has_https_redirect = True

        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            (HARDCODED_SECRET_PATTERN, "hardcoded_secret", "high",
             "hardcoded secret in Quart app — use environment variables"),
            ...
            (SSTI_PATTERN, "ssti", "high",
             "render_template_string with user input — SSTI risk, use templates"),
    ...
    def generate_hardened_template(self) -> str:
        """Scaffold a hardened Quart main.py entry template."""
        return """\
