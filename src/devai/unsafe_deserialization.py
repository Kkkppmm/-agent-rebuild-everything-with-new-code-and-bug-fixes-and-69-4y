...
_USER_INPUT_RE = re.compile(
    ...
    r"serialized|encoded|token|session|cookie|file|upload|stream)",
    re.IGNORECASE,
)
...
_UNSAFE_YAML_FUNCS = frozenset({"yaml.load", "yaml.load_all", "yaml.unsafe_load"})


@dataclass
class UnsafeDeserializationFinding:
    """An unsafe deserialization call pattern."""
...
def _has_safe_yaml_loader(node: ast.Call) -> bool:
    for kw in node.keywords:
        if kw.arg == "Loader" and isinstance(kw.value, ast.Attribute):
            attr = kw.value.attr
            if attr in {"SafeLoader", "CSafeLoader", "BaseLoader"}:
                return True
    return False
...
class _UnsafeDeserializationVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if name in _UNSAFE_LOAD_FUNCS:
            user_arg = node.args and _looks_like_user_input(node.args[0])
            severity = "critical" if user_arg else "high"
            self._add(
                node,
                "unsafe_deserialization",
                severity,
                f"{name} can execute arbitrary code — use JSON or safe loaders only",
                call=name,
            )

        if name in _UNSAFE_YAML_FUNCS and not _has_safe_yaml_loader(node):
            user_arg = node.args and _looks_like_user_input(node.args[0])
            severity = "critical" if user_arg else "high"
            self._add(
                node,
        ...
        if name.endswith(".loads") and name.split(".")[0] in {"pickle", "marshal", "dill"}:
            user_arg = node.args and _looks_like_user_input(node.args[0])
            severity = "critical" if user_arg else "high"
            self._add(
                node,
