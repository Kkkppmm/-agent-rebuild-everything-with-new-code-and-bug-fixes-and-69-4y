...
DEFINE_SECRET_PATTERN = re.compile(
    r"(?:API[_-]?SECRET|SECRET|TOKEN|PASSWORD|API[_-]?KEY)\s*:\s*['\"][^'\"${}]+['\"]",
    re.IGNORECASE,
)
...
CSRF_DISABLED_PATTERN = re.compile(
    r"(?:checkOrigin|csrf)\s*:\s*false",
    re.IGNORECASE,
)
PUBLIC_ENV_SECRET_PATTERN = re.compile(
    r"(?:PUBLIC_|VITE_)(?:API[_-]?KEY|SECRET|PASSWORD|TOKEN)\s*[=:]\s*['\"][^'\"${}]+['\"]",
    re.IGNORECASE,
)
...
def _is_vite_config_file(path: Path) -> bool:
    ...
    if pkg.is_file():
        try:
            ...
            if isinstance(deps, dict) and any(
                key in deps
                for key in ("@builder.io/qwik", "@builder.io/qwik-city", "qwik")
            ):
                return True
...
class QwikAnalyzer:
    ...
    Scans qwik.config.* and vite.config.* (in Qwik projects) for hardcoded
    secrets, exposed dev/preview servers, permissive CORS/origin settings,
    internal proxy targets, disabled CSRF checks, and production sourcemaps.
    """
    ...
    def configs(self) -> list[Path]:
        ...
        if is_qwik:
            ...
            for pattern in ("vite.config.*",):
                for path in sorted(self.root.rglob(pattern)):
                    if path.is_file() and path not in found and _is_vite_config_file(path):
                        found.append(path)
    ...
    def _scan_line(
        ...
        for section in ("server", "preview", "define", "env", "adapter", "build"):
            if section in stripped and ":" in stripped:
                if section not in info.sections:
                    info.sections.append(section)
                if hasattr(info, f"has_{section}"):
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (DEFINE_SECRET_PATTERN, "define_secret", "high",
             "secret in define block — use import.meta.env with runtime secrets"),
            (PUBLIC_ENV_SECRET_PATTERN, "public_env_secret", "high",
             "secret in PUBLIC_/VITE_ env — client-exposed values must not be secrets"),
            ...
            (ORIGIN_WILDCARD_PATTERN, "origin_wildcard", "medium",
             "server origin set to * — any origin may access the dev server"),
            ...
            (TRUSTED_ORIGINS_WILDCARD_PATTERN, "trusted_origins_wildcard", "medium",
             "trustedOrigins includes * — CSRF protection weakened"),
            (FS_ALLOW_PERMISSIVE_PATTERN, "fs_allow_permissive", "medium",
             "server.fs.allow is permissive — dev server may read sensitive paths"),
