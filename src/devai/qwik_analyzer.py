...
QWIK_CITY_PLUGIN_PATTERN = re.compile(
    r"(?:qwikCity|@builder\.io/qwik-city|@qwik\.dev/qwik-city)",
    re.IGNORECASE,
)
...
def _vite_has_qwik_plugin(path: Path) -> bool:
    try:
        text = path.read_text(encoding="utf-8", errors="replace")
        return QWIK_CITY_PLUGIN_PATTERN.search(text) is not None
    except OSError:
        return False
...
class QwikAnalyzer:
    ...
    Scans qwik.config.* and vite.config.* (when Qwik City is detected) for
    hardcoded secrets, exposed dev/preview servers, internal proxy targets,
    and production sourcemaps.
    """
    ...
    def configs(self) -> list[Path]:
        ...
        if is_qwik:
            for name in VITE_CONFIG_NAMES:
                path = self.root / name
                if path.is_file() and path not in found and _vite_has_qwik_plugin(path):
                    found.append(path)
            for pattern in ("vite.config.*",):
                for path in sorted(self.root.rglob(pattern)):
                    if (
                        ...
                        and _is_vite_config_file(path)
                        and _vite_has_qwik_plugin(path)
                    ):
                        found.append(path)
        return found
    ...
    def _scan_line(
        ...
        for section in ("server", "preview", "qwikCity", "plugins"):
