...
class SecurityScanner:
    ...
    weak crypto usage, log injection risks, SSRF risks, open redirect risks,
    and unsafe deserialization into one report.
  """
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("open_redirect", SecurityScanCategory("open_redirect", 100, 0, "")).findings:
            recs.append("Validate redirect targets against an allowlist of trusted domains and paths.")
        if by_name.get("unsafe_deserialization", SecurityScanCategory("unsafe_deserialization", 100, 0, "")).findings:
            recs.append("Replace pickle/marshal with JSON and use yaml.safe_load or Loader=SafeLoader.")
        return recs
