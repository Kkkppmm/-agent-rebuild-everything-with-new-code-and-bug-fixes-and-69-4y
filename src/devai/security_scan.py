...
class SecurityScanner:
    ...
    def _unsafe_deser_analyzer(self) -> UnsafeDeserializationAnalyzer:
        if self._unsafe_deser is None:
            self._unsafe_deser = UnsafeDeserializationAnalyzer(str(self.root), ignore_dirs=self.ignore_dirs)
    ...
    def to_context(self, limit: int = 40) -> str:
        ...
        if "unsafe_deserialization" in self.checks:
            lines.extend(["## Unsafe deserialization", self._unsafe_deser_analyzer().to_context(limit=limit), ""])
        if "jwt_security" in self.checks:
