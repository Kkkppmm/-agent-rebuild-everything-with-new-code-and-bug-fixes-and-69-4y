...
class SecurityScanner:
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get(
            ...
            recs.append(
                "Set DEBUG=False and use INFO or WARNING log levels in production — "
                "avoid console handlers and sensitive log formats."
            )
        if by_name.get(
            "insecure_cors_settings",
            ...
            recs.append(
                ...
                "never combine credentials with wildcard or allow-all origins."
            )
        if by_name.get(
            "insecure_storage_settings",
            ...
            recs.append(
                "Use cloud object storage with private ACLs — "
                "avoid FileSystemStorage and hardcoded AWS credentials in production."
            )
        return recs
