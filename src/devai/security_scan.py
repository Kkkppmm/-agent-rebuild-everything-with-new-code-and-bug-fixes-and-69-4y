...
class SecurityScanner:
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get(
            ...
            recs.append(
                "Use environment variables for JWT secrets, enable signature verification, "
                "avoid algorithm 'none', and set reasonable token expiration times."
            )
        if by_name.get(
            "insecure_webhook_settings",
            ...
            recs.append(
                "Use HTTPS webhook URLs, configure signing secrets, and always verify "
        ...
        if by_name.get(
            ...
            recs.append(
                "Disable GraphQL introspection and playground in production, turn off debug "
                "mode, and set a reasonable query depth limit."
            )
        return recs
