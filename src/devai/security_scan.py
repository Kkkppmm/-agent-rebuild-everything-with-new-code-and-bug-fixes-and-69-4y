...
class SecurityScanner:
    ...
    insecure random usage, path-traversal risks, weak crypto, log injection,
    and command injection into one report.
  """
    ...
    def __init__(
        ...
        *,
        ...
        self._command_injection: CommandInjectionAnalyzer | None = None
    ...
    def _command_injection_analyzer(self) -> CommandInjectionAnalyzer:
        if self._command_injection is None:
            self._command_injection = CommandInjectionAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._command_injection
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("log_injection", SecurityScanCategory("log_injection", 100, 0, "")).findings:
            recs.append("Use structured logging with %s placeholders instead of f-strings in log calls.")
        if by_name.get("command_injection", SecurityScanCategory("command_injection", 100, 0, "")).findings:
            recs.append("Pass subprocess arguments as a list and avoid shell=True with user input.")
        return recs
    ...
    def scan(self) -> SecurityScanReport:
        ...
        if "command_injection" in self.checks:
            analyzer = self._command_injection_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
    ...
    def to_context(self, limit: int = 40) -> str:
        ...
        if "command_injection" in self.checks:
            lines.extend(
                ["## Command injection", self._command_injection_analyzer().to_context(limit=limit), ""]
            )
        return "\n".join(lines).rstrip()
    ...
    def command_injection(self) -> CommandInjectionAnalyzer:
        ...
        return self._command_injection_analyzer()
