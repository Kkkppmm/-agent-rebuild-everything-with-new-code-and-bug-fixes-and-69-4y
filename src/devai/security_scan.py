...
class SecurityScanner:
    ...
    log injection, SSRF, unsafe deserialization, open redirects, insecure TLS,
    XSS, CORS misconfiguration, insecure cookies, and mass assignment into one report.
  """
    ...
    def __init__(
        ...
        *,
        ...
        self._mass_assign: MassAssignmentAnalyzer | None = None
    ...
    def _mass_assign_analyzer(self) -> MassAssignmentAnalyzer:
        if self._mass_assign is None:
            self._mass_assign = MassAssignmentAnalyzer(str(self.root), ignore_dirs=self.ignore_dirs)
        return self._mass_assign
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("insecure_tls", SecurityScanCategory("insecure_tls", 100, 0, "")).findings:
            recs.append("Enable TLS certificate verification — never use verify=False or unverified SSL contexts.")
        if by_name.get("xss", SecurityScanCategory("xss", 100, 0, "")).findings:
            recs.append("Avoid mark_safe and |safe filters — sanitize user input before rendering HTML.")
        if by_name.get("cors_misconfig", SecurityScanCategory("cors_misconfig", 100, 0, "")).findings:
            recs.append("Replace wildcard CORS origins with an explicit domain allowlist.")
        if by_name.get("insecure_cookies", SecurityScanCategory("insecure_cookies", 100, 0, "")).findings:
            recs.append("Set secure=True, httponly=True, and samesite='Lax' on all session cookies.")
        if by_name.get("mass_assignment", SecurityScanCategory("mass_assignment", 100, 0, "")).findings:
            recs.append("Whitelist allowed fields instead of passing request data directly to ORM create/update.")
        return recs
    ...
    def scan(self) -> SecurityScanReport:
        ...
        if "mass_assignment" in self.checks:
            analyzer = self._mass_assign_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
    ...
    def to_context(self, limit: int = 40) -> str:
        ...
        if "cors_misconfig" in self.checks:
            lines.extend(["## CORS misconfig", self._cors_analyzer().to_context(limit=limit), ""])
        if "insecure_cookies" in self.checks:
            lines.extend(["## Insecure cookies", self._cookies_analyzer().to_context(limit=limit), ""])
        if "mass_assignment" in self.checks:
            lines.extend(["## Mass assignment", self._mass_assign_analyzer().to_context(limit=limit), ""])
        return "\n".join(lines).rstrip()
    ...
    @property
    def xss(self) -> XssVulnerabilityAnalyzer:
    ...
    def mass_assignment(self) -> MassAssignmentAnalyzer:
        ...
        return self._mass_assign_analyzer()
