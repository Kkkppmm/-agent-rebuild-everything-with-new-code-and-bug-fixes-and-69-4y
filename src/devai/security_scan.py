...
class SecurityScanner:
    ...
    and open redirect checks into one report.
  """
    ...
    def _unsafe_deser_analyzer(self) -> UnsafeDeserializationAnalyzer:
        if self._unsafe_deser is None:
            self._unsafe_deser = UnsafeDeserializationAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._unsafe_deser
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("unsafe_deserialization", SecurityScanCategory("unsafe_deserialization", 100, 0, "")).findings:
            recs.append("Never deserialize untrusted data with pickle, marshal, or yaml.load — use safe_load or JSON.")
    ...
    def scan(self) -> SecurityScanReport:
        ...
        if "unsafe_deserialization" in self.checks:
            analyzer = self._unsafe_deser_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
        ...
        if "unsafe_deserialization" in self.checks:
            lines.extend(
                [
                    ...
                    self._unsafe_deser_analyzer().to_context(limit=limit),
                    "",
                ]
            )
        if "open_redirect" in self.checks:
    ...
    def unsafe_deserialization(self) -> UnsafeDeserializationAnalyzer:
        ...
        return self._unsafe_deser_analyzer()
