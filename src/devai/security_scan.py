...
class SecurityScanner:
    ...
    and XXE-vulnerable XML parsing into one report.
  """
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("redos", SecurityScanCategory("redos", 100, 0, "")).findings:
            recs.append("Simplify regex patterns and avoid nested quantifiers to prevent ReDoS attacks.")
        if by_name.get("jwt_security", SecurityScanCategory("jwt_security", 100, 0, "")).findings:
            recs.append("Always verify JWT signatures and never accept the 'none' algorithm.")
        ...
        if by_name.get("xxe", SecurityScanCategory("xxe", 100, 0, "")).findings:
            recs.append("Use defusedxml for parsing untrusted XML and disable external entity resolution.")
        return recs
    ...
    def to_context(self, limit: int = 40) -> str:
        ...
        if "unsafe_deserialization" in self.checks:
            lines.extend([
                ...
                self._deserialization_analyzer().to_context(limit=limit),
                "",
            ])
        if "insecure_tls" in self.checks:
