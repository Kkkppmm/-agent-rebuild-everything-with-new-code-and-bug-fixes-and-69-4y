...
from devai.insecure_swagger_settings import InsecureSwaggerSettingsAnalyzer
from devai.insecure_elasticsearch_settings import InsecureElasticsearchSettingsAnalyzer
...
CHECK_NAMES = (
    ...
    "insecure_swagger_settings",
    "insecure_elasticsearch_settings",
...
class SecurityScanner:
    ...
    def __init__(
        ...
        *,
        ...
        self._insecure_swagger_settings: InsecureSwaggerSettingsAnalyzer | None = None
        self._insecure_elasticsearch_settings: InsecureElasticsearchSettingsAnalyzer | None = None
    ...
    def _insecure_oauth_settings_analyzer(self) -> InsecureOAuthSettingsAnalyzer:
        if self._insecure_oauth_settings is None:
            self._insecure_oauth_settings = InsecureOAuthSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._insecure_oauth_settings

    def _insecure_swagger_settings_analyzer(self) -> InsecureSwaggerSettingsAnalyzer:
        if self._insecure_swagger_settings is None:
            self._insecure_swagger_settings = InsecureSwaggerSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._insecure_swagger_settings

    def _insecure_elasticsearch_settings_analyzer(self) -> InsecureElasticsearchSettingsAnalyzer:
        if self._insecure_elasticsearch_settings is None:
            self._insecure_elasticsearch_settings = InsecureElasticsearchSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._insecure_elasticsearch_settings

    def _insecure_redis_settings_analyzer(self) -> InsecureRedisSettingsAnalyzer:
        if self._insecure_redis_settings is None:
            self._insecure_redis_settings = InsecureRedisSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._insecure_redis_settings

    def _insecure_mongo_settings_analyzer(self) -> InsecureMongoSettingsAnalyzer:
        if self._insecure_mongo_settings is None:
            self._insecure_mongo_settings = InsecureMongoSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
        return self._insecure_mongo_settings

    def _insecure_kafka_settings_analyzer(self) -> InsecureKafkaSettingsAnalyzer:
        if self._insecure_kafka_settings is None:
            self._insecure_kafka_settings = InsecureKafkaSettingsAnalyzer(
                str(self.root), ignore_dirs=self.ignore_dirs
            )
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get(
            ...
            recs.append(
                "Disable GraphQL introspection and playground/GraphiQL in production — "
                "restrict schema exposure to authenticated clients."
            )
        if by_name.get(
            "insecure_webhook_settings",
            ...
            recs.append(
                "Enable webhook signature verification and load webhook secrets from "
                "environment variables with 32+ character random values."
            )
        if by_name.get(
            "insecure_jwt_settings",
            ...
            recs.append(
                "Enable JWT signature verification, use RS256 or HS256 with a strong secret, "
                "and load JWT keys from environment variables."
            )
        if by_name.get(
            "insecure_oauth_settings",
            SecurityScanCategory("insecure_oauth_settings", 100, 0, ""),
        ).findings:
            recs.append(
                "Enable OAuth PKCE and state validation, disable insecure transport, "
                "and load client secrets from environment variables."
            )
        if by_name.get(
            "insecure_swagger_settings",
            SecurityScanCategory("insecure_swagger_settings", 100, 0, ""),
        ).findings:
            recs.append(
                "Disable public Swagger/OpenAPI schema access in production and require "
                "authentication for API documentation endpoints."
            )
        if by_name.get(
            "insecure_elasticsearch_settings",
            SecurityScanCategory("insecure_elasticsearch_settings", 100, 0, ""),
        ).findings:
            recs.append(
                "Use HTTPS with authentication for Elasticsearch, enable certificate verification, "
                "and load credentials from environment variables."
            )
        if by_name.get(
            "insecure_redis_settings",
            SecurityScanCategory("insecure_redis_settings", 100, 0, ""),
        ).findings:
            recs.append(
                "Enable TLS (rediss://) and authentication for Redis, and load passwords from "
                "environment variables."
            )
        if by_name.get(
            "insecure_mongo_settings",
            SecurityScanCategory("insecure_mongo_settings", 100, 0, ""),
        ).findings:
            recs.append(
                "Use authenticated MongoDB connections with TLS (mongodb+srv://) and load "
                "credentials from environment variables."
            )
        if by_name.get(
            "insecure_kafka_settings",
            SecurityScanCategory("insecure_kafka_settings", 100, 0, ""),
        ).findings:
            recs.append(
    ...
    def scan(self) -> SecurityScanReport:
        ...
        if "insecure_oauth_settings" in self.checks:
            analyzer = self._insecure_oauth_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_oauth_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
        ...
        if "insecure_swagger_settings" in self.checks:
            analyzer = self._insecure_swagger_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_swagger_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
        ...
        if "insecure_elasticsearch_settings" in self.checks:
            analyzer = self._insecure_elasticsearch_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_elasticsearch_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
        ...
        if "insecure_redis_settings" in self.checks:
            analyzer = self._insecure_redis_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_redis_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
        ...
        if "insecure_mongo_settings" in self.checks:
            analyzer = self._insecure_mongo_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_mongo_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
        ...
        if "insecure_kafka_settings" in self.checks:
            analyzer = self._insecure_kafka_settings_analyzer()
            findings = analyzer.analyze()
            categories.append(
                SecurityScanCategory(
                    name="insecure_kafka_settings",
                    score=analyzer.health_score(),
                    findings=len(findings),
                    summary=analyzer.summary().splitlines()[0],
                )
    ...
    def to_context(self, limit: int = 40) -> str:
        ...
        if "insecure_swagger_settings" in self.checks:
            lines.extend(
                [
                    "## Insecure Swagger settings",
                    self._insecure_swagger_settings_analyzer().to_context(limit=limit),
                    "",
                ]
            )
        if "insecure_elasticsearch_settings" in self.checks:
            lines.extend(
                [
                    "## Insecure Elasticsearch settings",
                    self._insecure_elasticsearch_settings_analyzer().to_context(limit=limit),
                    "",
                ]
            )
        if "insecure_redis_settings" in self.checks:
            lines.extend(
                [
                    "## Insecure Redis settings",
                    self._insecure_redis_settings_analyzer().to_context(limit=limit),
                    "",
                ]
            )
        if "insecure_mongo_settings" in self.checks:
            lines.extend(
                [
                    "## Insecure MongoDB settings",
                    self._insecure_mongo_settings_analyzer().to_context(limit=limit),
                    "",
                ]
            )
        if "insecure_kafka_settings" in self.checks:
    ...
    @property
    def insecure_swagger_settings(self) -> InsecureSwaggerSettingsAnalyzer:
        """Underlying insecure-Swagger-settings analyzer."""
        return self._insecure_swagger_settings_analyzer()

    @property
    def insecure_elasticsearch_settings(self) -> InsecureElasticsearchSettingsAnalyzer:
        """Underlying insecure-Elasticsearch-settings analyzer."""
        return self._insecure_elasticsearch_settings_analyzer()

    @property
    def insecure_redis_settings(self) -> InsecureRedisSettingsAnalyzer:
        """Underlying insecure-Redis-settings analyzer."""
        return self._insecure_redis_settings_analyzer()

    @property
    def insecure_mongo_settings(self) -> InsecureMongoSettingsAnalyzer:
        """Underlying insecure-MongoDB-settings analyzer."""
        return self._insecure_mongo_settings_analyzer()
