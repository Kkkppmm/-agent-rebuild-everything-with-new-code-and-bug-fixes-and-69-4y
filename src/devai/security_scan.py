...
class SecurityScanner:
    ...
    log injection, SSRF, unsafe deserialization, open redirects, ReDoS, JWT security,
    and insecure TLS into one report.
  """
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("redos", SecurityScanCategory("redos", 100, 0, "")).findings:
            recs.append("Simplify regex patterns with nested quantifiers or use atomic groups/timeouts.")
        if by_name.get("jwt_security", SecurityScanCategory("jwt_security", 100, 0, "")).findings:
            recs.append("Always verify JWT signatures with an explicit algorithms allowlist.")
        if by_name.get("insecure_tls", SecurityScanCategory("insecure_tls", 100, 0, "")).findings:
            recs.append("Never disable TLS certificate verification in production HTTP clients.")
        return recs
