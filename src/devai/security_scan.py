...
class SecurityScanner:
    ...
    information disclosure, header injection, and mass assignment checks into one report.
    """
    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get("information_disclosure", SecurityScanCategory("information_disclosure", 100, 0, "")).findings:
            recs.append("Return generic error messages and avoid exposing tracebacks or internal object state.")
        if by_name.get("header_injection", SecurityScanCategory("header_injection", 100, 0, "")).findings:
            recs.append("Sanitize user input used in HTTP headers and validate redirect URLs.")
        if by_name.get("mass_assignment", SecurityScanCategory("mass_assignment", 100, 0, "")).findings:
            recs.append("Use explicit allowlists when mapping request data to model attributes.")
        return recs
