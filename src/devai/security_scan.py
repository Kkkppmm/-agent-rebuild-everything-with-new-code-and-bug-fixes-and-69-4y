    ...
    def _build_recommendations(self, categories: list[SecurityScanCategory]) -> list[str]:
        ...
        if by_name.get(
            ...
            recs.append(
                ...
                "add depth-limit middleware, and protect views with authentication."
            )
        if by_name.get(
            "insecure_webhook_settings",
            ...
            recs.append(
                "Load webhook secrets from environment, enable signature verification, "
                "use HTTPS endpoints, and avoid csrf_exempt on webhook handlers."
            )
        if by_name.get(
            "insecure_jwt_settings",
            ...
            recs.append(
                "Use environment-backed JWT signing keys, reject the 'none' algorithm, "
                "enable signature verification, use short token expiry, and pass tokens via headers."
            )
        return recs
