...
PIPFILE_STANDALONE_NAMES = ("Pipfile", "Pipfile.lock")
...
class PipfileFinding:
    """A security or best-practice issue in a Pipfile configuration file."""
...
@dataclass
class PipfileInfo:
    """Parsed metadata about a Pipfile configuration file."""
...
@dataclass
class PipfileStats:
    """Aggregate Pipfile analysis statistics."""
...
def _is_pipfile_file(path: Path) -> bool:
    """Return True if the path looks like a Pipfile configuration file."""
    name = path.name
    if name in PIPFILE_STANDALONE_NAMES:
        return True
    return False
...
class PipfileAnalyzer:
    """Audit Pipfile configuration for security issues.
    ...
    def configs(self) -> list[Path]:
        """Return Pipfile configuration paths found in the project."""
        found: list[Path] = []
        for path in sorted(self.root.rglob("*")):
            if path.is_file() and _is_pipfile_file(path):
                found.append(path)
        return found
    ...
    def _analyze_file(self, path: Path) -> tuple[list[PipfileFinding], PipfileInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    PipfileFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Pipfile config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    PipfileFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Pipfile config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if AWS_ACCESS_KEY_PATTERN.search(line):
                findings.append(
                    PipfileFinding(
                        kind="aws_access_key",
                        severity="high",
                        message="AWS access key in Pipfile config — use credential helpers or secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:packages|dev-packages)", stripped, re.IGNORECASE)
        ...
        if path.name in ("pyproject.toml", "Pipfile") and not lock_path.exists():
            findings.append(
                PipfileFinding(
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# Pipfile — hardened Pipenv defaults
[[source]]
...
[packages]
# Pin versions and commit Pipfile.lock for reproducible installs
"""
    ...
    def summary(self) -> str:
        ...
        stats = self.stats
        if stats.configs == 0:
            return "Pipfile configs: none found"
        return (
            f"Pipfile configs: {stats.configs} config(s), "
