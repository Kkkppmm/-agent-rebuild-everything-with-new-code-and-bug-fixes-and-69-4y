"""SetuptoolsAnalyzer — audit setup.py, setup.cfg, and setuptools pyproject.toml for security and build hardening."""
...
SETUPTOOLS_STANDALONE_NAMES = ("setup.py", "setup.cfg")

SETUPTOOLS_MARKER_PATTERN = re.compile(
    r"(?:^\[build-system\].*setuptools|^\[tool\.setuptools\]|setup\(|setuptools\.setup)",
...
def _is_setuptools_file(path: Path) -> bool:
    ...
    name = path.name
    if name in SETUPTOOLS_STANDALONE_NAMES:
        return True
    if name == "pyproject.toml":
        try:
            head = path.read_text(encoding="utf-8", errors="replace")[:8192]
            if SETUPTOOLS_MARKER_PATTERN.search(head):
                return True
        except OSError:
            pass
    return False
...
class SetuptoolsAnalyzer:
    ...
    def _analyze_file(self, path: Path) -> tuple[list[SetuptoolsFinding], SetuptoolsInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    SetuptoolsFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Setuptools config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    SetuptoolsFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Setuptools config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:install_requires|requires|dependencies)", stripped, re.IGNORECASE)
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# setup.cfg — hardened setuptools defaults
[metadata]
# Store credentials via environment variables or CI secret stores

[options]
# Pin dependencies in requirements.txt or pyproject.toml [project.dependencies]
