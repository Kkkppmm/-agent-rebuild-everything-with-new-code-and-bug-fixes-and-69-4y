...
BOTTLE_IMPORT_PATTERN = re.compile(
    r"(?:from\s+bottle\s+import|import\s+bottle|bottle\.(?:route|run|default_app|install|hook|request|response|static_file|template))",
    re.IGNORECASE,
)
BOTTLE_ROUTE_PATTERN = re.compile(
    r"@(?:app\.|bottle\.)?route\s*\(|@route\s*\(",
    re.IGNORECASE,
)
...
CORS_WILDCARD_PATTERN = re.compile(
    ...
    r"response\.set_header\s*\(\s*['\"]Access-Control-Allow-Origin['\"]\s*,\s*['\"]\*['\"]",
    re.IGNORECASE,
)
...
DEBUG_MODE_PATTERN = re.compile(
    r"(?:run|bottle\.run)\s*\([^)]*debug\s*=\s*True|debug\s*=\s*True",
    re.IGNORECASE,
)
DANGEROUS_ROUTE_PATTERN = re.compile(
    r"@(?:app\.|bottle\.)?route\s*\([^)]*\)\s*\n\s*def\s+(?:admin|debug|internal)",
    re.IGNORECASE,
)
...
STATIC_USER_INPUT_PATTERN = re.compile(
    r"static_file\s*\([^)]*(?:request\.|bottle\.request|getattr|\.params|\.query)",
    re.IGNORECASE,
)
BIND_ALL_PATTERN = re.compile(
    r"(?:run|bottle\.run)\s*\([^)]*host\s*=\s*['\"]0\.0\.0\.0['\"]|host\s*=\s*['\"]0\.0\.0\.0['\"]",
    re.IGNORECASE,
)
SSTI_PATTERN = re.compile(
    r"template\s*\([^)]*(?:request\.|bottle\.request|\.params|\.query|\.forms)",
    re.IGNORECASE,
)
OPEN_REDIRECT_PATTERN = re.compile(
    r"redirect\s*\(\s*(?:request\.|bottle\.request)",
    re.IGNORECASE,
)
RELOADER_PATTERN = re.compile(
    r"(?:run|bottle\.run)\s*\([^)]*reloader\s*=\s*True|reloader\s*=\s*True",
    re.IGNORECASE,
)
...
def _contains_bottle(text: str) -> bool:
    return bool(
        ...
        or "bottle.run" in text
        or "default_app()" in text
    )
...
class BottleAnalyzer:
    ...
    Scans Bottle entry files, routes, and config for hardcoded secrets,
    debug mode, open CORS, SSRF targets, shell command execution, SSTI,
    ...
    def _scan_line(
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            (HARDCODED_SECRET_PATTERN, "hardcoded_secret", "high",
             "hardcoded secret in Bottle app — use environment variables or config files"),
            ...
            (STATIC_USER_INPUT_PATTERN, "static_file_user_input", "high",
             "static_file from user input — validate paths to prevent traversal"),
            ...
            (SSTI_PATTERN, "ssti", "high",
             "template() with user input — validate or use autoescaping to prevent SSTI"),
    ...
    def generate_hardened_template(self) -> str:
        """Scaffold a hardened Bottle main.py entry template."""
        return """\
