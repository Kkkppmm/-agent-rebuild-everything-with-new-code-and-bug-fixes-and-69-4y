...
BOTTLE_IMPORT_PATTERN = re.compile(
    r"(?:from\s+bottle|import\s+bottle|bottle\.(?:Bottle|run|route|static_file|template)\s*\()",
    re.IGNORECASE,
)
BOTTLE_ROUTE_PATTERN = re.compile(
    r"@(?:app\.)?(?:route|get|post|put|delete|patch|hook)\s*\(|"
    r"(?:app|bottle)\.route\s*\(",
    re.IGNORECASE,
)
...
CORS_WILDCARD_PATTERN = re.compile(
    r"(?:CORS|bottle_cors)[^)]*(?:allow_origin|allow_origins|Access-Control-Allow-Origin)\s*=\s*['\"]\*['\"]|"
    r"(?:allow_origin|allow_origins|Access-Control-Allow-Origin)\s*=\s*['\"]\*['\"]",
    re.IGNORECASE,
)
...
PROXY_INTERNAL_PATTERN = re.compile(
    ...
    r"(?:urlopen|urllib\.request\.urlopen|requests\.(?:get|post))\s*\([^)]*['\"]https?://(?:10\.|"
...
DEBUG_MODE_PATTERN = re.compile(
    ...
    r"(?:debug|DEBUG|reloader)\s*=\s*True",
    re.IGNORECASE,
)
DANGEROUS_ROUTE_PATTERN = re.compile(
    r"@(?:app\.)?(?:route|get|post)\s*\(\s*['\"](?:/)?(?:admin|debug|internal)",
    re.IGNORECASE,
)
...
STATIC_FILE_USER_INPUT_PATTERN = re.compile(
    r"static_file\s*\([^)]*(?:request\.|get\(|params|query)",
    re.IGNORECASE,
)
...
OPEN_REDIRECT_PATTERN = re.compile(
    r"redirect\s*\(\s*(?:request\.|get\(|params)",
    re.IGNORECASE,
)
TEMPLATE_NO_ESCAPE_PATTERN = re.compile(
    r"template\s*\([^)]*noescape\s*=\s*True",
    re.IGNORECASE,
)
...
class BottleAnalyzer:
    ...
    Scans Bottle entry files, routes, and plugins for hardcoded secrets,
    debug mode, SSRF targets, shell command execution, path traversal via
  static_file, and unprotected admin routes.
    """
    ...
    def _scan_line(
        ...
        if "CORS(" in stripped or "bottle_cors" in stripped.lower():
            info.has_cors = True
        if any(k in stripped for k in ("BasicAuth", "auth_basic", "require_auth", "login_required")):
            info.has_auth = True
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (DEBUG_MODE_PATTERN, "debug_mode", "medium",
             "debug/reloader enabled — disable in production"),
            ...
            (STATIC_FILE_USER_INPUT_PATTERN, "static_file_user_input", "high",
             "static_file with user input — validate paths to prevent traversal"),
            (TEMPLATE_NO_ESCAPE_PATTERN, "template_noescape", "high",
             "template with noescape=True — enable auto-escaping to prevent XSS"),
