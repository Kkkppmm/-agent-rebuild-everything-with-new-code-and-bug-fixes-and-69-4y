...
SITE_METADATA_SECRET_PATTERN = re.compile(
    r"(?:apiKey|apiSecret|authToken|accessToken|clientSecret)\s*:\s*['\"][^'\"${}]+['\"]",
    re.IGNORECASE,
)
GRAPHQL_PLAYGROUND_PATTERN = re.compile(
    r"graphqlPlayground\s*:\s*true",
    re.IGNORECASE,
)
...
PROXY_INTERNAL_PATTERN = re.compile(
    r"proxy\s*:\s*\{[^}]*(?:target|url)\s*:\s*['\"]https?://(?:10\.|192\.168\.|"
    r"172\.(?:1[6-9]|2\d|3[01])\.|169\.254\.)",
...
def _looks_like_gatsby_project(root: Path) -> bool:
    if any((root / name).exists() for name in GATSBY_CONFIG_NAMES):
        return True
    if any(root.glob("gatsby-config.*")):
        return True
...
class GatsbyAnalyzer:
    ...
    Scans gatsby-config.* files for hardcoded secrets in siteMetadata,
    GraphQL playground exposure, developMiddleware hooks, internal proxy
    targets, and insecure siteUrl values.
    """
    ...
    def _scan_line(
        ...
        for section in ("siteMetadata", "plugins", "proxy", "develop", "developMiddleware"):
            if section in stripped and ":" in stripped:
                if section not in info.sections:
                    info.sections.append(section)
                if section == "siteMetadata":
                    info.has_site_metadata = True
                elif section == "plugins":
                    info.has_plugins = True
                elif section == "proxy":
                    info.has_proxy = True
                elif section in ("develop", "developMiddleware"):
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (SITE_METADATA_SECRET_PATTERN, "site_metadata_secret", "high",
             "secret in siteMetadata — use GATSBY_* environment variables at build time"),
            ...
            (DEVELOP_MIDDLEWARE_PATTERN, "develop_middleware", "medium",
             "developMiddleware hook defined — review for auth bypass or SSRF"),
            ...
            (TRAILING_SLASH_ALWAYS_PATTERN, "trailing_slash_always", "low",
             "trailingSlash set to always — may cause redirect loops with misconfigured CDN"),
        ]
...
module.exports = {
  ...
  trailingSlash: 'never',
  developMiddleware: undefined,
  proxy: undefined,
  plugins: [
    {
      resolve: 'gatsby-plugin-graphql-playground',
