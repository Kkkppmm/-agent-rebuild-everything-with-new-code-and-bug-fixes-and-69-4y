...
HARDCODED_SECRET_PATTERN = re.compile(
    ...
    r"private[_-]?key|client[_-]?secret|auth[_-]?token|bearer|sessionSecret)\s*[=:]\s*"
    r"[\"']?[^\"'\s${}][^\"'<]*[\"']?",
    re.IGNORECASE,
)
...
ENV_SECRET_PATTERN = re.compile(
    r"(?:API[_-]?KEY|SECRET|PASSWORD|TOKEN|CREDENTIAL|CLIENT[_-]?SECRET|SESSION_SECRET)\s*:\s*['\"][^'\"${}]+['\"]",
    re.IGNORECASE,
)
...
SERVER_BUNDLE_EXTERNAL_PATTERN = re.compile(
    r"serverDependenciesToBundle\s*:\s*(?:false|['\"]none['\"])",
    re.IGNORECASE,
)
WATCH_IGNORED_PATTERN = re.compile(
    r"watchPaths\s*:\s*\[[^\]]*['\"]\*['\"]",
    re.IGNORECASE | re.DOTALL,
)
FUTURE_V2_DEV_PATTERN = re.compile(
    r"v2_dev\s*:\s*true",
    re.IGNORECASE,
)
DEV_SERVER_PORT_EXPOSED_PATTERN = re.compile(
    r"devServerPort\s*:\s*(?:3000|8080|80)\b",
    re.IGNORECASE,
)
...
def _looks_like_remix_project(root: Path) -> bool:
    ...
    if pkg.is_file():
        try:
            ...
            if isinstance(deps, dict) and ("@remix-run/node" in deps or "remix" in deps):
                return True
...
class RemixAnalyzer:
    ...
    internal proxy targets, permissive filesystem access, and production
    sourcemaps.
    """
    ...
    def _scan_line(
        ...
        for section in ("server", "vite", "future", "serverModuleFormat", "publicPath"):
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (WATCH_IGNORED_PATTERN, "watch_paths_wildcard", "medium",
             "watchPaths includes * — may watch sensitive directories"),
            (SERVER_BUNDLE_EXTERNAL_PATTERN, "server_bundle_external", "low",
             "serverDependenciesToBundle disabled — may bundle untrusted server deps"),
            (FUTURE_V2_DEV_PATTERN, "v2_dev_enabled", "low",
             "future.v2_dev enabled — experimental dev behavior in production builds"),
            (DEV_SERVER_PORT_EXPOSED_PATTERN, "dev_server_port", "low",
             "default dev server port — ensure production does not expose dev port"),
        ]
