  ...
  in a few lines.
    ...
    def from_project(
        ...
        *,
        ...
        project_path = Path(path or Path.cwd())
        config_path = find_config_file(project_path)
        if config_path and not use_mock:
            config = load_config_file(config_path)
            return cls.from_config(config, project_path=project_path)
        return cls.create(use_mock=use_mock, project_path=project_path)

    def git(self, path: str | Path | None = None) -> "GitContext":
        """Create a GitContext for the project."""
        from devai.git_context import GitContext

        return GitContext(path or self.project_path or Path.cwd())

    def review_git(self, preset: str = "pr-review") -> list[ProgramResult]:
        """Run a preset using git context (diff, branch, status)."""
        context = self.git().to_context()
        return self.run(preset, context)

    def trace(self, name: str = "runtime") -> "DevTrace":
        """Create a DevTrace for timing program steps."""
        from devai.trace import DevTrace

        return DevTrace(name=name)
