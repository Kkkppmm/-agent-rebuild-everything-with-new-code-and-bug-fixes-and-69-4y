  ...
  in a few lines.
    ...
    def from_env(
        ...
        **overrides: Any,
        ...
        """Bootstrap a runtime from environment variables (see :meth:`DevAIConfig.from_env`)."""
        resolved_path = project_path or os.environ.get("DEVAI_PROJECT_PATH")
        config = DevAIConfig.from_env(**overrides)
        if config.api_key == "mock":
            return cls.create(use_mock=True, project_path=resolved_path)
        return cls.from_config(config, project_path=resolved_path)
    ...
    def report(
        self,
        data: list[ProgramResult] | WorkflowResult,
        *,
        title: str | None = None,
        format: str = "markdown",
    ) -> str:
        ...
        if isinstance(data, WorkflowResult):
            report = ProgramReport.from_workflow(data)
        else:
            report = ProgramReport.from_program_results(
                data,
                title=title or "Program Report",
                program_name=title or "program",
            )
        if format == "json":
