...
def cmd_tflint_audit(args: argparse.Namespace) -> None:
    ...
    analyzer = TflintAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "hadolint-audit",
        help="Audit Hadolint configs for ignored root-user/latest-tag/secret rules, permissive thresholds, and insecure registries",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "tflint-audit",
        help="Audit TFLint configs for disabled security rules, unpinned plugins, and disabled_by_default",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened TFLint configuration template",
    )
    p.set_defaults(func=cmd_tflint_audit)

    p = sub.add_parser(
        "actionlint-audit",
        help="Audit actionlint configs for broad workflow ignores, shellcheck suppressions, and runner labels",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
