...
def cmd_docsify_audit(args: argparse.Namespace) -> None:
    from devai.docsify_analyzer import DocsifyAnalyzer

    analyzer = DocsifyAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "docsify-audit",
        help="Audit Docsify index.html for executeScript, remote plugins, CDN scripts, and hardcoded tokens",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
