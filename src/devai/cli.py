...
def cmd_starlight_audit(args: argparse.Namespace) -> None:
    from devai.starlight_analyzer import StarlightAnalyzer

    analyzer = StarlightAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "starlight-audit",
        help="Audit Astro Starlight configs for hardcoded secrets, remote head scripts, and unsafe expressiveCode",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
