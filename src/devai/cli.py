...
def cmd_sphinx_audit(args: argparse.Namespace) -> None:
    from devai.sphinx_analyzer import SphinxAnalyzer

    analyzer = SphinxAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "mkdocs-audit",
        help="Audit MkDocs configs for HTTP URLs, public dev servers, and secrets",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened mkdocs.yml template",
    )
    p.set_defaults(func=cmd_mkdocs_audit)

    p = sub.add_parser(
        "sphinx-audit",
        help="Audit Sphinx conf.py for exec patterns, secrets, and weak defaults",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened conf.py template snippet",
    )
    p.set_defaults(func=cmd_sphinx_audit)
