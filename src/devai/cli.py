...
def cmd_redos(args: argparse.Namespace) -> None:
    from devai.redos import ReDoSAnalyzer

    analyzer = ReDoSAnalyzer(args.directory)
    if args.context:
        print(analyzer.to_context())
        return
    print(analyzer.summary())
...
def cmd_timing_attack(args: argparse.Namespace) -> None:
    from devai.timing_attack import TimingAttackAnalyzer

    analyzer = TimingAttackAnalyzer(args.directory)
    if args.context:
        print(analyzer.to_context())
        return
    print(analyzer.summary())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("redos", help="Detect regex denial-of-service patterns")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser("timing-attack", help="Detect insecure secret comparisons")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.set_defaults(func=cmd_timing_attack)
