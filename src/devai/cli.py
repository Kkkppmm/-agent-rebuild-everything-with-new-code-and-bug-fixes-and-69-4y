...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    for path in result.skipped:
        print(f"Skipped (exists): {path.relative_to(result.root)}", file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("init", help="Scaffold a new DevAI project with config and programs")
    p.add_argument(
        "path",
        nargs="?",
        default=".",
    ...
    p.add_argument("--provider", default="openai", help="Provider name for the config template")
    p.add_argument("--model", default="gpt-4o-mini", help="Model name for the config template")
    p.add_argument("--force", action="store_true", help="Overwrite existing scaffold files")
    p.add_argument("--no-schedule", action="store_true", help="Skip devai-schedule.yaml")
    p.add_argument("--no-starter", action="store_true", help="Skip devai_main.py starter script")
