...
def cmd_cors_misconfig(args: argparse.Namespace) -> None:
...
def cmd_jwt_security(args: argparse.Namespace) -> None:
    from devai.jwt_security import JWTSecurityAnalyzer

    analyzer = JWTSecurityAnalyzer(args.directory)
    if args.context:
        print(analyzer.to_context())
        return
    print(analyzer.summary())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("cors-misconfig", help="Detect permissive CORS configuration")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.set_defaults(func=cmd_cors_misconfig)

    p = sub.add_parser("jwt-security", help="Detect insecure JWT handling")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
