...
def cmd_invoke_audit(args: argparse.Namespace) -> None:
    from devai.invoke_analyzer import InvokeAnalyzer

    analyzer = InvokeAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_doit_audit(args: argparse.Namespace) -> None:
    from devai.doit_analyzer import DoitAnalyzer

    analyzer = DoitAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_taskipy_audit(args: argparse.Namespace) -> None:
    from devai.taskipy_analyzer import TaskipyAnalyzer

    analyzer = TaskipyAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_commitizen_audit(args: argparse.Namespace) -> None:
    from devai.commitizen_analyzer import CommitizenAnalyzer

    analyzer = CommitizenAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_semantic_release_audit(args: argparse.Namespace) -> None:
    from devai.semantic_release_analyzer import SemanticReleaseAnalyzer

    analyzer = SemanticReleaseAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_isort_audit(args: argparse.Namespace) -> None:
    from devai.isort_analyzer import IsortAnalyzer

    analyzer = IsortAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_flake8_audit(args: argparse.Namespace) -> None:
    from devai.flake8_analyzer import Flake8Analyzer

    analyzer = Flake8Analyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_pyright_audit(args: argparse.Namespace) -> None:
    from devai.pyright_analyzer import PyrightAnalyzer

    analyzer = PyrightAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_basedpyright_audit(args: argparse.Namespace) -> None:
    from devai.basedpyright_analyzer import BasedpyrightAnalyzer

    analyzer = BasedpyrightAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_mkdocs_audit(args: argparse.Namespace) -> None:
    from devai.mkdocs_analyzer import MkDocsAnalyzer

    analyzer = MkDocsAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_hugo_audit(args: argparse.Namespace) -> None:
    from devai.hugo_analyzer import HugoAnalyzer

    analyzer = HugoAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_vitepress_audit(args: argparse.Namespace) -> None:
    from devai.vitepress_analyzer import VitePressAnalyzer

    analyzer = VitePressAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_pylint_audit(args: argparse.Namespace) -> None:
    ...
    analyzer = PylintAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_golangci_audit(args: argparse.Namespace) -> None:
    from devai.golangci_analyzer import GolangciLintAnalyzer

    analyzer = GolangciLintAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_rubocop_audit(args: argparse.Namespace) -> None:
    from devai.rubocop_analyzer import RuboCopAnalyzer

    analyzer = RuboCopAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_markdownlint_audit(args: argparse.Namespace) -> None:
    from devai.markdownlint_analyzer import MarkdownlintAnalyzer

    analyzer = MarkdownlintAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_tsconfig_audit(args: argparse.Namespace) -> None:
    from devai.tsconfig_analyzer import TsconfigAnalyzer

    analyzer = TsconfigAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_hono_audit(args: argparse.Namespace) -> None:
    from devai.hono_analyzer import HonoAnalyzer

    analyzer = HonoAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_nestjs_audit(args: argparse.Namespace) -> None:
    from devai.nestjs_analyzer import NestJSAnalyzer

    analyzer = NestJSAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_flask_audit(args: argparse.Namespace) -> None:
    from devai.flask_analyzer import FlaskAnalyzer

    analyzer = FlaskAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_falcon_audit(args: argparse.Namespace) -> None:
    ...
    analyzer = FalconAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_cherrypy_audit(args: argparse.Namespace) -> None:
    from devai.cherrypy_analyzer import CherryPyAnalyzer

    analyzer = CherryPyAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_bottle_audit(args: argparse.Namespace) -> None:
    from devai.bottle_analyzer import BottleAnalyzer

    analyzer = BottleAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_pyramid_audit(args: argparse.Namespace) -> None:
    from devai.pyramid_analyzer import PyramidAnalyzer

    analyzer = PyramidAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_robyn_audit(args: argparse.Namespace) -> None:
    from devai.robyn_analyzer import RobynAnalyzer

    analyzer = RobynAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_blacksheep_audit(args: argparse.Namespace) -> None:
    from devai.blacksheep_analyzer import BlacksheepAnalyzer

    analyzer = BlacksheepAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_gradio_audit(args: argparse.Namespace) -> None:
    from devai.gradio_analyzer import GradioAnalyzer

    analyzer = GradioAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_chainlit_audit(args: argparse.Namespace) -> None:
    from devai.chainlit_analyzer import ChainlitAnalyzer

    analyzer = ChainlitAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_langchain_audit(args: argparse.Namespace) -> None:
    from devai.langchain_analyzer import LangChainAnalyzer

    analyzer = LangChainAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_solid_audit(args: argparse.Namespace) -> None:
    from devai.solid_analyzer import SolidAnalyzer

    analyzer = SolidAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_nuxt_audit(args: argparse.Namespace) -> None:
    ...
    analyzer = NuxtAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def cmd_webpack_audit(args: argparse.Namespace) -> None:
    ...
    analyzer = WebpackAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "invoke-audit",
        help="Audit tasks.py for sudo usage, warn_only, prompt=False, insecure indexes, and dangerous commands",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "doit-audit",
        help="Audit dodo.py for sudo usage, ignore=True, shell=True, insecure indexes, and dangerous commands",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened dodo.py template",
    )
    p.set_defaults(func=cmd_doit_audit)

    p = sub.add_parser(
        "taskipy-audit",
        help="Audit pyproject.toml [tool.taskipy] for use_vars=true, sudo usage, insecure indexes, and dangerous commands",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened pyproject.toml [tool.taskipy] template",
    )
    p.set_defaults(func=cmd_taskipy_audit)

    p = sub.add_parser(
        "commitizen-audit",
        help="Audit Commitizen configs for bump hooks, unsigned tags, custom version providers, and dangerous commands",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        ...
        help="Audit python-semantic-release configs for tokens, dangerous build commands, and unsafe release settings",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened pyproject.toml [tool.semantic_release] template",
    )
    p.set_defaults(func=cmd_semantic_release_audit)
    ...
    p = sub.add_parser(
        "isort-audit",
        help="Audit isort configs for skip patterns, honor_noqa=false, and Black profile conflicts",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened isort configuration template",
    )
    p.set_defaults(func=cmd_isort_audit)

    p = sub.add_parser(
        "flake8-audit",
        help="Audit Flake8 configs for broad ignores, disabled S rules, and source exclusions",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Flake8 configuration template",
    )
    p.set_defaults(func=cmd_flake8_audit)

    p = sub.add_parser(
        "pyright-audit",
        help="Audit Pyright configs for relaxed type checking and insecure paths",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "basedpyright-audit",
        help="Audit Basedpyright configs for relaxed type checking and disabled rules",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Basedpyright configuration template",
    )
    p.set_defaults(func=cmd_basedpyright_audit)
    ...
    p = sub.add_parser(
        "mkdocs-audit",
        help="Audit MkDocs configs for exposed dev servers, unsafe plugins, and remote assets",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "docusaurus-audit",
        help="Audit Docusaurus configs for Algolia keys, broken-link policy, and remote scripts",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Docusaurus configuration template",
    )
    p.set_defaults(func=cmd_docusaurus_audit)

    p = sub.add_parser(
        "hugo-audit",
        help="Audit Hugo configs for Goldmark unsafe mode, module imports, and remote scripts",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Hugo configuration template",
    )
    p.set_defaults(func=cmd_hugo_audit)

    p = sub.add_parser(
        "vitepress-audit",
        help="Audit VitePress configs for markdown.unsafe, Algolia keys, and remote scripts",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened VitePress configuration template",
    )
    p.set_defaults(func=cmd_vitepress_audit)

    p = sub.add_parser(
        "pylint-audit",
        help="Audit Pylint configs for broad disables, unsafe init-hook, and security rule suppression",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Pylint configuration template",
    )
    p.set_defaults(func=cmd_pylint_audit)

    p = sub.add_parser(
        "golangci-audit",
        help="Audit golangci-lint configs for disabled gosec, broad skip patterns, and gosec excludes",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened golangci-lint configuration template",
    )
    p.set_defaults(func=cmd_golangci_audit)

    p = sub.add_parser(
        "rubocop-audit",
        help="Audit RuboCop configs for disabled Security/* cops, broad Exclude patterns, and remote inherit_from",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened RuboCop configuration template",
    )
    p.set_defaults(func=cmd_rubocop_audit)
    ...
    p = sub.add_parser(
        "yamllint-audit",
        help="Audit yamllint configs for disabled truthy/key-duplicates checks, relaxed extends, and broad ignores",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "hadolint-audit",
        help="Audit Hadolint configs for ignored security rules, wildcard suppressions, and permissive thresholds",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "markdownlint-audit",
        help="Audit markdownlint configs for disabled inline-HTML checks, wildcard suppressions, and broad ignores",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "tsconfig-audit",
        help="Audit tsconfig/jsconfig files for disabled strict mode, secrets, and broad excludes",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "qwik-audit",
        help="Audit Qwik City vite.config.* for exposed dev servers, internal proxies, and adapter leaks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "gatsby-audit",
        help="Audit gatsby-config.* for HTTP site URLs, GraphQL playground, and plugin credential leaks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened gatsby-config.ts template",
    )
    p.set_defaults(func=cmd_gatsby_audit)

    p = sub.add_parser(
        "hono-audit",
        help="Audit Hono apps for hardcoded auth, open CORS, insecure cookies, and wrangler secret leaks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "nestjs-audit",
        help="Audit NestJS apps for hardcoded JWT secrets, open CORS, disabled validation, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened NestJS main.ts entry template",
    )
    p.set_defaults(func=cmd_nestjs_audit)

    p = sub.add_parser(
        "fastapi-audit",
        help="Audit FastAPI apps for hardcoded secrets, open CORS, exposed docs, debug mode, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened FastAPI main.py entry template",
    )
    p.set_defaults(func=cmd_fastapi_audit)

    p = sub.add_parser(
        "flask-audit",
        help="Audit Flask apps for hardcoded secrets, open CORS, debug mode, SSTI, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Flask app.py entry template",
    )
    p.set_defaults(func=cmd_flask_audit)

    p = sub.add_parser(
        "django-audit",
        help="Audit Django apps for hardcoded secrets, DEBUG mode, wildcard ALLOWED_HOSTS, CSRF bypass, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    ...
    p = sub.add_parser(
        "sanic-audit",
        help="Audit Sanic apps for hardcoded secrets, open CORS, debug mode, SSTI, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    ...
    p = sub.add_parser(
        "falcon-audit",
        help="Audit Falcon apps for hardcoded secrets, open CORS, debug mode, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    ...
    p = sub.add_parser(
        "cherrypy-audit",
        help="Audit CherryPy apps for hardcoded secrets, development mode, open CORS, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened CherryPy main.py entry template",
    )
    p.set_defaults(func=cmd_cherrypy_audit)

    p = sub.add_parser(
        "bottle-audit",
        help="Audit Bottle apps for hardcoded secrets, debug mode, open CORS, SSTI, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Bottle main.py entry template",
    )
    p.set_defaults(func=cmd_bottle_audit)

    p = sub.add_parser(
        "pyramid-audit",
        help="Audit Pyramid apps for hardcoded secrets, debug toolbar, disabled CSRF, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    ...
    p = sub.add_parser(
        "robyn-audit",
        help="Audit Robyn apps for hardcoded secrets, debug mode, open CORS, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Robyn main.py entry template",
    )
    p.set_defaults(func=cmd_robyn_audit)

    p = sub.add_parser(
        "blacksheep-audit",
        help="Audit BlackSheep apps for hardcoded secrets, reload mode, open CORS, shell commands, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened BlackSheep main.py entry template",
    )
    p.set_defaults(func=cmd_blacksheep_audit)

    p = sub.add_parser(
        "streamlit-audit",
        help="Audit Streamlit apps for hardcoded secrets, unsafe HTML, disabled XSRF, and committed secrets files",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Streamlit app.py entry template",
    )
    p.set_defaults(func=cmd_streamlit_audit)

    p = sub.add_parser(
        "gradio-audit",
        help="Audit Gradio apps for hardcoded secrets, public share links, disabled auth, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened Gradio app.py entry template",
    )
    p.set_defaults(func=cmd_gradio_audit)

    p = sub.add_parser(
        "chainlit-audit",
        help="Audit Chainlit apps for hardcoded secrets, missing auth, permissive CORS, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    ...
    p = sub.add_parser(
        "langchain-audit",
        help="Audit LangChain agents and RAG chains for hardcoded API keys, dangerous tools, SQL injection, and SSRF risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("-v", "--verbose", action="store_true", help="Print each finding")
    p.add_argument("--context", action="store_true", help="Print LLM context summary")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened LangChain RAG agent entry template",
    )
    p.set_defaults(func=cmd_langchain_audit)

    p = sub.add_parser(
        "sveltekit-audit",
        help="Audit svelte.config.* for disabled CSRF checks, adapter leaks, and SSRF fetch targets",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "solid-audit",
        help="Audit app.config.* and vite.config.* for SolidStart session secret leaks, disabled CSP, and SSRF proxies",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened app.config.ts template for SolidStart",
    )
    p.set_defaults(func=cmd_solid_audit)

    p = sub.add_parser(
        "nuxt-audit",
        help="Audit nuxt.config.* for runtimeConfig leaks, internal proxies, and production devtools",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p = sub.add_parser(
        "webpack-audit",
        help="Audit webpack.config.* for exposed dev servers, secrets, and production sourcemaps",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened webpack.config.js template",
    )
    p.set_defaults(func=cmd_webpack_audit)
