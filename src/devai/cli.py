...
def cmd_pep723_audit(args: argparse.Namespace) -> None:
    from devai.pep723_analyzer import Pep723Analyzer

    analyzer = Pep723Analyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "pep723-audit",
        help="Audit PEP 723 inline script metadata blocks for unpinned deps and supply-chain risks",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.add_argument(
        "--generate-template",
        action="store_true",
        help="Print a hardened PEP 723 inline script metadata template",
    )
    p.set_defaults(func=cmd_pep723_audit)

    p = sub.add_parser(
        "ty-audit",
        help="Audit ty.toml and pyproject.toml [tool.ty] for disabled rules and relaxed type checking",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
