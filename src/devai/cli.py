...
def cmd_k8s_audit(args: argparse.Namespace) -> None:
    from devai.k8s_analyzer import K8sManifestAnalyzer

    analyzer = K8sManifestAnalyzer(args.directory)
    if args.generate_template:
        print(analyzer.generate_hardened_template())
        return
    if args.context:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "k8s-audit",
        help="Audit Kubernetes manifests for security and cluster best practices",
    )
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    ...
    p.set_defaults(func=cmd_k8s_audit)
