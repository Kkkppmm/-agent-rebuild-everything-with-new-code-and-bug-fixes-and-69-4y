...
def cmd_code_smell(args: argparse.Namespace) -> None:
    assistant = _get_assistant(args)
    code = _read_input(args.code)
    print(assistant.code_smell(code, context=args.context))


def cmd_openapi(args: argparse.Namespace) -> None:
    assistant = _get_assistant(args)
    code = _read_input(args.code)
    print(assistant.openapi(code, requirements=args.requirements))
...
def cmd_ci(args: argparse.Namespace) -> None:
    ...
    if args.gate == "security":
        report = reporter.security_gate(code)
    elif args.gate == "review":
        report = reporter.code_review_gate(code)
    elif args.gate == "diff":
        if not diff:
            diff = git_diff()
        report = reporter.review_diff(diff)
    else:
        report = reporter.full_ci_gate(code, diff=diff)
    ...
    if args.format == "json":
        ...
        print(json.dumps(report.to_actions_annotations(), indent=2))
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("code-smell", help="Identify code smells")
    p.add_argument("code", help="Code or file path")
    p.add_argument("--context", default="", help="Additional context")
    p.set_defaults(func=cmd_code_smell)

    p = sub.add_parser("openapi", help="Review or generate OpenAPI spec")
    p.add_argument("code", help="API code or description")
    p.add_argument("--requirements", default="", help="API requirements")
    p.set_defaults(func=cmd_openapi)

    p = sub.add_parser("ci", help="Generate CI report for GitHub Actions")
    p.add_argument("code", help="Code or file path")
    p.add_argument("--diff", help="Diff text or file path")
    p.add_argument(
        "--gate",
        choices=["full", "review", "security", "diff"],
        default="full",
        help="CI gate type",
    )
    p.add_argument(
        "--format",
        choices=["markdown", "json", "annotations"],
        default="markdown",
        help="Output format",
    )
