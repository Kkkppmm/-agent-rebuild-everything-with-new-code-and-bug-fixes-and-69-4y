...
def cmd_xss(args: argparse.Namespace) -> None:
    ...
    analyzer = XSSAnalyzer(args.directory)
    if args.context:
        print(analyzer.to_context())
        return
    print(analyzer.summary())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("redos", help="Detect regex patterns vulnerable to ReDoS")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
    p.set_defaults(func=cmd_redos)

    p = sub.add_parser("xss", help="Detect reflected XSS risks in web handlers")
    p.add_argument("directory", nargs="?", default=".", help="Project directory")
    p.add_argument("--context", action="store_true", help="Output LLM-ready context")
    p.add_argument("--verbose", "-v", action="store_true", help="List all findings")
