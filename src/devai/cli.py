...
def cmd_health(args: argparse.Namespace) -> None:
    result = check_health(use_mock=args.mock)
...
def cmd_doctor(args: argparse.Namespace) -> None:
    result = run_doctor(project_path=args.project)
    print(result.format_report())
    if not result.healthy:
        sys.exit(1)
...
def cmd_config_init(args: argparse.Namespace) -> None:
    ...
    if path.exists() and not args.force:
        ...
        sys.exit(1)
    path.write_text(config_file_template(), encoding="utf-8")
    print(f"Created {path}")


def cmd_config_show(args: argparse.Namespace) -> None:
    config_path = find_config_file(args.project)
...
def cmd_benchmark(args: argparse.Namespace) -> None:
    if args.mock:
        result = benchmark_mock(requests=args.requests)
    else:
        runtime = DevRuntime.create(use_mock=False)
        result = BenchmarkRunner(runtime.client).run(
            prompt=args.prompt,
...
def cmd_git_review(args: argparse.Namespace) -> None:
    runtime = DevRuntime.create(use_mock=args.mock, project_path=args.project)
    results = runtime.review_git(args.preset)
...
def cmd_report(args: argparse.Namespace) -> None:
    ...
    if args.program:
        program = DevProgram.from_file(args.program, runtime.assistant)
        results = program.run(context)
        report = ProgramReport.from_program(program.name, results)
    else:
        results = runtime.run(args.preset or "pre-commit", context)
        report = ProgramReport.from_program(args.preset or "pre-commit", results)
...
def cmd_trace_demo(args: argparse.Namespace) -> None:
    trace = DevTrace(name="demo")
    runtime = DevRuntime.create(use_mock=True)
    with trace.span("load-preset", preset=args.preset):
        with trace.span("run-program"):
            runtime.run(args.preset, {"code": _read_input(args.code)})
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--project", help="Project directory to check")
    p.set_defaults(func=cmd_doctor)

    p = sub.add_parser("config-init", help="Create a .devai.yaml config file")
    p.add_argument("--path", default=".devai.yaml", help="Config file path")
    p.add_argument("--force", action="store_true", help="Overwrite existing file")
    p.set_defaults(func=cmd_config_init)

    p = sub.add_parser("config-show", help="Show the project DevAI config file")
    p.add_argument("--project", help="Project directory to search")
    p.set_defaults(func=cmd_config_show)

    p = sub.add_parser("benchmark", help="Benchmark LLM latency")
    p.add_argument("--prompt", default="Say hello in one word.")
    p.add_argument("--requests", type=int, default=5)
    p.set_defaults(func=cmd_benchmark)

    p = sub.add_parser("git-review", help="Run a preset with git context")
    p.add_argument("--preset", default="pr-review", help="Preset to run")
    p.add_argument("--project", help="Git repository path")
    p.set_defaults(func=cmd_git_review)

    p = sub.add_parser("report", help="Run a program and export results")
    p.add_argument("--program", help="Program JSON/YAML file")
    ...
    p = sub.add_parser("trace-demo", help="Demonstrate DevTrace timing spans")
    p.add_argument("--preset", default="pre-commit", help="Preset to trace")
    p.add_argument("code", help="Code or file path")
