...
class DevAI:
    ...
    def pixi(self, path: str | Path = ".", **kwargs: Any) -> PixiAnalyzer:
        """Audit Pixi pixi.toml for secrets, insecure channels, unpinned deps, and missing lockfiles."""
        return PixiAnalyzer(str(path), **kwargs)
    ...
    def scikit_build(self, path: str | Path = ".", **kwargs: Any) -> ScikitBuildAnalyzer:
        """Audit scikit-build-core pyproject.toml and CMakeLists.txt for secrets, path traversal, and CMake risks."""
    ...
    def doit(self, path: str | Path = ".", **kwargs: Any) -> DoitAnalyzer:
        """Audit dodo.py for sudo usage, ignore=True, shell=True, insecure indexes, and dangerous commands."""
        return DoitAnalyzer(str(path), **kwargs)

    def taskipy(self, path: str | Path = ".", **kwargs: Any) -> TaskipyAnalyzer:
        """Audit pyproject.toml [tool.taskipy] for use_vars=true, sudo usage, insecure indexes, and dangerous commands."""
        return TaskipyAnalyzer(str(path), **kwargs)
    ...
    def semantic_release(self, path: str | Path = ".", **kwargs: Any) -> SemanticReleaseAnalyzer:
        """Audit python-semantic-release configs for tokens, dangerous build commands, and unsafe release settings."""
        return SemanticReleaseAnalyzer(str(path), **kwargs)
    ...
    def mkdocs(self, path: str | Path = ".", **kwargs: Any) -> MkDocsAnalyzer:
        """Audit mkdocs.yml for exposed dev servers, unsafe plugins, remote assets, and relaxed validation."""
        return MkDocsAnalyzer(str(path), **kwargs)

    def docusaurus(self, path: str | Path = ".", **kwargs: Any) -> DocusaurusAnalyzer:
        """Audit docusaurus.config.* for Algolia keys, broken-link policy, remote scripts, and unsafe HTML."""
    ...
    def hugo(self, path: str | Path = ".", **kwargs: Any) -> HugoAnalyzer:
        """Audit Hugo hugo.toml/config.* for Goldmark unsafe mode, module imports, and remote scripts."""
        return HugoAnalyzer(str(path), **kwargs)

    def vitepress(self, path: str | Path = ".", **kwargs: Any) -> VitePressAnalyzer:
        """Audit .vitepress/config.* for markdown.unsafe, Algolia keys, remote scripts, and exposed dev servers."""
    ...
    def openapi_spec(self, path: str | Path = ".", **kwargs: Any) -> OpenAPIAnalyzer:
        """Audit OpenAPI/Swagger specs for insecure servers, weak auth, and sensitive paths."""
        return OpenAPIAnalyzer(str(path), **kwargs)

    def buf(self, path: str | Path = ".", **kwargs: Any) -> BufAnalyzer:
        """Audit buf.yaml and buf.gen.yaml for disabled lint/breaking rules and insecure plugins."""
        return BufAnalyzer(str(path), **kwargs)
