...
class DevAI:
    ...
    def from_env(
        ...
        **kwargs: Any,
        ...
        Reads ``DEVAI_PROVIDER``, ``DEVAI_API_KEY``, ``DEVAI_MODEL``, and
        related settings. Set ``DEVAI_PROVIDER=mock`` for offline usage.
        """
        from devai.core.config import DevAIConfig

        config = DevAIConfig.from_env(**kwargs)
        if config.api_key == "mock":
            return cls.mock(project_path=project_path)
        provider = os.environ.get("DEVAI_PROVIDER", "openai").lower()
        if provider == "ollama":
            return cls.ollama(model=config.model, base_url=config.base_url, project_path=project_path)
        return cls.openai(api_key=config.api_key, model=config.model, project_path=project_path)
