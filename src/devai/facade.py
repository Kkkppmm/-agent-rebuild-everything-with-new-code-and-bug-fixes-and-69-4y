...
class DevAI:
    ...
    def from_env(
        ...
        **kwargs: Any,
        ...
        Set ``DEVAI_API_KEY`` or ``OPENAI_API_KEY``, optionally ``DEVAI_PROVIDER``,
        ``DEVAI_MODEL``, and ``DEVAI_BASE_URL``. Use ``DEVAI_MOCK=1`` for local dev.
        """
        return cls(DevRuntime.from_env(project_path=project_path, **kwargs))
