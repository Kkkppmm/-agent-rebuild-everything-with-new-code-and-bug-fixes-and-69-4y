...
class DevAI:
    ...
    def cibuildwheel(self, path: str | Path = ".", **kwargs: Any) -> CibuildwheelAnalyzer:
        """Audit cibuildwheel pyproject.toml for secrets, shell injection, and insecure build hooks."""
        return CibuildwheelAnalyzer(str(path), **kwargs)

    def maturin(self, path: str | Path = ".", **kwargs: Any) -> MaturinAnalyzer:
        """Audit maturin pyproject.toml and Cargo.toml for secrets, unpinned git deps, and insecure URLs."""
        return MaturinAnalyzer(str(path), **kwargs)

    def scikit_build(self, path: str | Path = ".", **kwargs: Any) -> ScikitBuildAnalyzer:
        """Audit scikit-build-core pyproject.toml and CMakeLists.txt for secrets and insecure downloads."""
