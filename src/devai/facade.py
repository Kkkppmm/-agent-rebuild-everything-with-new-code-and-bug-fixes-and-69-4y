...
class DevAI:
    ...
    def bun(self, path: str | Path = ".", **kwargs: Any) -> BunAnalyzer:
        """Audit bunfig.toml, bun.lock, and bun.lockb for secrets, insecure registries, and unpinned deps."""
        return BunAnalyzer(str(path), **kwargs)

    def deno(self, path: str | Path = ".", **kwargs: Any) -> DenoAnalyzer:
        """Audit deno.json/jsonc and import maps for overly broad permissions and insecure imports."""
    ...
    def cypress(self, path: str | Path = ".", **kwargs: Any) -> CypressAnalyzer:
        """Audit cypress.config.* and cypress.env.json for chromeWebSecurity, secrets, and insecure baseUrl."""
