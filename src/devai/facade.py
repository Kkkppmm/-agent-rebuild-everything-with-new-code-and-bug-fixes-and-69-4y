...
class DevAI:
    ...
    def hugo(self, path: str | Path = ".", **kwargs: Any) -> HugoAnalyzer:
        """Audit Hugo hugo.toml/config.* for exposed servers, unsafe Goldmark, and remote scripts."""
        return HugoAnalyzer(str(path), **kwargs)

    def vitepress(self, path: str | Path = ".", **kwargs: Any) -> VitePressAnalyzer:
        """Audit VitePress .vitepress/config.* for exposed servers, fs.allow, and remote head scripts."""
        return VitePressAnalyzer(str(path), **kwargs)

    def docusaurus(self, path: str | Path = ".", **kwargs: Any) -> DocusaurusAnalyzer:
        """Audit docusaurus.config.* for ignored broken links, remote scripts, and hardcoded secrets."""
