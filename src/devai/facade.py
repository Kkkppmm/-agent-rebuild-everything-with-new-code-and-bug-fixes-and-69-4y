...
class DevAI:
    ...
    def unsafe_deserialization(self, path: str | Path = ".", **kwargs: Any) -> UnsafeDeserializationAnalyzer:
        """Detect unsafe deserialization of untrusted data (pickle, yaml.load, etc.)."""
    ...
    def xss(self, path: str | Path = ".", **kwargs: Any) -> XSSAnalyzer:
        """Detect cross-site scripting risks in HTML rendering."""
        return XSSAnalyzer(str(path), **kwargs)

    def xxe(self, path: str | Path = ".", **kwargs: Any) -> XXEAnalyzer:
        """Detect XML external entity injection risks in XML parsing."""
