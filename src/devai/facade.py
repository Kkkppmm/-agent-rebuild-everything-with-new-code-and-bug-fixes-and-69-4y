...
class DevAI:
    ...
    def taskfile(self, path: str | Path = ".", **kwargs: Any) -> TaskfileAnalyzer:
        """Audit Taskfile.yml/yaml for secrets, dangerous shell commands, and insecure URLs."""
        return TaskfileAnalyzer(str(path), **kwargs)

    def direnv(self, path: str | Path = ".", **kwargs: Any) -> DirenvAnalyzer:
        """Audit .envrc and direnv.toml for secrets, remote sources, and unsafe env loading."""
        return DirenvAnalyzer(str(path), **kwargs)

    def just(self, path: str | Path = ".", **kwargs: Any) -> JustAnalyzer:
        """Audit justfile recipes for secrets, dangerous shell commands, and insecure URLs."""
