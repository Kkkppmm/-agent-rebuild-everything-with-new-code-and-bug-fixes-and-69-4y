...
class DevAI:
    ...
    def sveltekit(self, path: str | Path = ".", **kwargs: Any) -> SvelteKitAnalyzer:
        """Audit svelte.config.* for disabled CSRF, exposed dev servers, CSP off, and adapter credential leaks."""
        return SvelteKitAnalyzer(str(path), **kwargs)

    def gatsby(self, path: str | Path = ".", **kwargs: Any) -> GatsbyAnalyzer:
        """Audit gatsby-config.* for siteMetadata secrets, internal proxies, GraphQL IDE exposure, and plugin secrets."""
        return GatsbyAnalyzer(str(path), **kwargs)

    def qwik(self, path: str | Path = ".", **kwargs: Any) -> QwikAnalyzer:
        """Audit qwik.config.* and vite.config.* for exposed servers, disabled origin checks, and adapter secrets."""
