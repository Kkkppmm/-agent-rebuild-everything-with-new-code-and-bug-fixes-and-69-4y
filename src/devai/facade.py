...
class DevAI:
    ...
    def gitlab_ci(self, path: str | Path = ".", **kwargs: Any) -> GitLabCIAnalyzer:
        """Audit GitLab CI/CD pipelines for secrets, privileged services, and unsafe scripts."""
        return GitLabCIAnalyzer(str(path), **kwargs)

    def jenkinsfile(self, path: str | Path = ".", **kwargs: Any) -> JenkinsfileAnalyzer:
        """Audit Jenkins pipelines for script injection, secrets, and privileged Docker."""
        return JenkinsfileAnalyzer(str(path), **kwargs)

    def ansible(self, path: str | Path = ".", **kwargs: Any) -> AnsibleAnalyzer:
        """Audit Ansible playbooks for hardcoded secrets and unsafe shell tasks."""
    ...
    def kubernetes(self, path: str | Path = ".", **kwargs: Any) -> K8sAnalyzer:
        """Audit Kubernetes manifests for privileged pods and missing resource limits."""
