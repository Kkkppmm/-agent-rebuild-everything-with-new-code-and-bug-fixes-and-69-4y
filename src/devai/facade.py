...
class DevAI:
    ...
    def isort(self, path: str | Path = ".", **kwargs: Any) -> IsortAnalyzer:
        """Audit isort configs for honor_noqa=false, skip patterns, and Black profile conflicts."""
        return IsortAnalyzer(str(path), **kwargs)

    def pylint(self, path: str | Path = ".", **kwargs: Any) -> PylintAnalyzer:
        """Audit pylint configs for init-hook, disable=all, unsafe-load-any-extension, and broad ignore patterns."""
