...
class DevAI:
    ...
    def remix(self, path: str | Path = ".", **kwargs: Any) -> RemixAnalyzer:
        """Audit remix.config.* for exposed dev servers, permissive watchPaths, and SSRF proxy targets."""
        return RemixAnalyzer(str(path), **kwargs)

    def sveltekit(self, path: str | Path = ".", **kwargs: Any) -> SvelteKitAnalyzer:
        """Audit svelte.config.* for disabled CSRF checks, permissive CSP, and adapter credential leaks."""
        return SvelteKitAnalyzer(str(path), **kwargs)

    def gatsby(self, path: str | Path = ".", **kwargs: Any) -> GatsbyAnalyzer:
        """Audit gatsby-config.* for siteMetadata secrets, GraphQL playground exposure, and internal proxies."""
        return GatsbyAnalyzer(str(path), **kwargs)

    def qwik(self, path: str | Path = ".", **kwargs: Any) -> QwikAnalyzer:
        """Audit Qwik City vite/qwik configs for exposed dev/preview servers and SSRF proxy targets."""
