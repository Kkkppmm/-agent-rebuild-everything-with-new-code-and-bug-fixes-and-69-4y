...
class DevAI:
    ...
    def streamlit(self, path: str | Path = ".", **kwargs: Any) -> StreamlitAnalyzer:
        """Audit Streamlit apps for hardcoded secrets, unsafe HTML, XSRF, and committed secrets risks."""
        return StreamlitAnalyzer(str(path), **kwargs)

    def gradio(self, path: str | Path = ".", **kwargs: Any) -> GradioAnalyzer:
        """Audit Gradio apps for hardcoded secrets, share links, missing auth, and SSRF risks."""
        return GradioAnalyzer(str(path), **kwargs)

    def chainlit(self, path: str | Path = ".", **kwargs: Any) -> ChainlitAnalyzer:
        """Audit Chainlit apps for hardcoded secrets, missing auth, CORS, file uploads, and SSRF risks."""
