...
HATCH_PYPROJECT_NAMES = ("pyproject.toml",)
HATCH_CONFIG_NAMES = ("hatch.toml",)
HATCH_MARKER_PATTERN = re.compile(
    r"(?:^\[tool\.hatch\]|^\[tool\.hatch\.|^\[tool\.hatch\.envs\.|"
    r"^\[tool\.hatch\.build\]|^\[tool\.hatch\.publish\]|^\[tool\.hatch\.version\]|"
    r"hatchling|hatch\s*=\s*\{)",
...
DANGEROUS_SCRIPT_PATTERN = re.compile(
    r"(?:pre-install|post-install|scripts)\s*=\s*\[",
    re.IGNORECASE,
)
INSECURE_ENV_VAR_PATTERN = re.compile(
    r"(?:HATCH_INDEX_USER|HATCH_INDEX_AUTH|TWINE_PASSWORD|TWINE_USERNAME)\s*=\s*"
    r"[\"'][^\"']+[\"']",
    re.IGNORECASE,
)
...
@dataclass
class HatchFinding:
    """A security or best-practice issue in a Hatch configuration file."""
...
@dataclass
class HatchInfo:
    """Parsed metadata about a Hatch configuration file."""
...
@dataclass
class HatchStats:
    """Aggregate Hatch analysis statistics."""
...
def _is_hatch_file(path: Path) -> bool:
    """Return True if the path looks like a Hatch configuration file."""
    name = path.name
    if name in HATCH_CONFIG_NAMES:
        return True
    if name in HATCH_PYPROJECT_NAMES:
        try:
            head = path.read_text(encoding="utf-8", errors="replace")[:8192]
            if HATCH_MARKER_PATTERN.search(head):
                return True
        except OSError:
            pass
    return False
...
class HatchAnalyzer:
    """Audit Hatch configuration for security issues.

    Scans pyproject.toml (with [tool.hatch]), hatch.toml, and related files
    ...
    curl-pipe-to-shell in scripts, disabled SSL verification, and publish
    credentials in plain text.
    """

    def __init__(self, root: str) -> None:
        self.root = Path(root)
        self._findings: list[HatchFinding] | None = None
        self._stats: HatchStats | None = None
        self._infos: list[HatchInfo] | None = None

    def configs(self) -> list[Path]:
        """Return Hatch configuration paths found in the project."""
        found: list[Path] = []
        for path in sorted(self.root.rglob("*")):
            if path.is_file() and _is_hatch_file(path):
                found.append(path)
        return found

    def _analyze_file(self, path: Path) -> tuple[list[HatchFinding], HatchInfo]:
        findings: list[HatchFinding] = []
        rel = str(path.relative_to(self.root))
        try:
            text = path.read_text(encoding="utf-8", errors="replace")
        except OSError:
            return findings, HatchInfo(path=rel)

        raw_lines = text.splitlines()
        info = HatchInfo(path=rel, lines=len(raw_lines), file_kind=_file_kind(path))
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            env_match = re.match(r"^\[tool\.hatch\.envs\.([^\]]+)\]", stripped, re.IGNORECASE)
            if env_match:
                info.envs.append(env_match.group(1))
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Hatch config — use HATCH_INDEX_AUTH or CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Hatch config — use keyring or HATCH_PUBLISH_* env vars",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if AWS_ACCESS_KEY_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        kind="aws_access_key",
                        severity="high",
                        message="AWS access key in Hatch config — use credential helpers or secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if INSECURE_ENV_VAR_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        ...
                        severity="high",
                        message="publish credentials in Hatch config — use env vars or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(
                        r"(?:dependencies|dev-dependencies|override-dependencies|sources|envs)",
                        stripped,
                        re.IGNORECASE,
                    )
            ...
            if CURL_PIPE_SHELL_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        ...
                        severity="high",
                        message="curl/wget piped to shell in Hatch config — vendor scripts with checksum verification",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if DANGEROUS_SCRIPT_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        ...
                        severity="medium",
                        message="install script hook in Hatch env — review for supply-chain risks",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
    ...
    def analyze(self) -> list[HatchFinding]:
        """Run analysis and return all findings."""
        if self._findings is not None:
            return self._findings

        findings: list[HatchFinding] = []
        infos: list[HatchInfo] = []
        paths = self.configs()
    ...
    def generate_hardened_config(self) -> str:
        """Scaffold a hardened hatch.toml snippet with secure defaults."""
        return """\
...
#   export HATCH_INDEX_AUTH=pypi-<token>
# Never commit tokens in pyproject.toml or hatch.toml

[tool.hatch.envs.default]
# Prefer pinned dependencies in [project] or requirements files
# Avoid curl-pipe-to-shell in pre-install/post-install scripts
"""
    ...
    def summary(self) -> str:
        ...
        stats = self.stats
        if stats.configs == 0:
            return "Hatch configs: none found"
        return (
            f"Hatch configs: {stats.configs} config(s), "
    ...
    def to_context(self) -> str:
        ...
        for info in self.infos:
            ...
            envs = ", ".join(info.envs[:8]) if info.envs else "none"
            lines.append(
                f"  - {info.path} ({info.file_kind}): "
                f"{len(info.dependencies)} dependency(ies), {len(info.envs)} env(s)"
            )
