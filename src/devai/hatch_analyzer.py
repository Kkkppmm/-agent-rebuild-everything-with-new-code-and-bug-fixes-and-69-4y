...
HATCH_STANDALONE_NAMES = ("hatch.toml",)

HATCH_MARKER_PATTERN = re.compile(
    r"(?:^\[tool\.hatch\]|^\[tool\.hatch\.|hatchling)",
...
def _is_hatch_file(path: Path) -> bool:
    ...
    name = path.name
    if name in HATCH_STANDALONE_NAMES:
        return True
...
class HatchAnalyzer:
    ...
    def _analyze_file(self, path: Path) -> tuple[list[HatchFinding], HatchInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Hatch config — use CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    HatchFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Hatch config — use environment variables or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and (
                    re.search(r"(?:dependencies|dev-dependencies|environments)", stripped, re.IGNORECASE)
    ...
    def generate_hardened_config(self) -> str:
        ...
        return """# hatch.toml — hardened Hatch defaults
[build]
# Use HTTPS PyPI mirrors only; never commit tokens in config files

[publish.index]
# Configure publishing credentials via HATCH_INDEX_* env vars
"""
