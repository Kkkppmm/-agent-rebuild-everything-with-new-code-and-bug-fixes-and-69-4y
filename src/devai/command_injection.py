...
_USER_INPUT_RE = re.compile(
    r"(request|user|input|cmd|command|shell|argv|args|param|query|"
    r"filename|path|name|host|url|addr|port|script|payload|data)",
    re.IGNORECASE,
)

_SHELL_FUNCS = frozenset({"system", "popen", "spawn", "spawnl", "spawnle", "spawnlp", "spawnlpe"})
...
def _is_shell_command_call(node: ast.Call) -> tuple[str, str, str] | None:
    """Return (pattern, severity, message) for risky shell command construction."""
    name = _call_name(node)

    if name in {"os.system", "os.popen"} or name in _SHELL_FUNCS:
        if node.args and _is_dynamic_string(node.args[0]):
            return (
                "dynamic_os_shell",
                "high",
                "Shell command built from user-controlled input — use subprocess with a list",
            )
        return None

    if name.startswith("subprocess.") or name.split(".")[-1] in _SUBPROCESS_FUNCS:
        if not node.args:
            return None
        first = node.args[0]
        if _has_shell_true(node) and _is_dynamic_string(first):
            return (
                "dynamic_subprocess_shell",
                "high",
                "subprocess shell command built from user input — pass argv as a list",
            )
        if isinstance(first, (ast.JoinedStr, ast.BinOp)) and not _has_shell_true(node):
            return (
                "dynamic_argv_string",
                "medium",
                "subprocess called with dynamically built string argument — verify escaping",
            )
        if isinstance(first, ast.Call):
            ...
            if isinstance(func, ast.Attribute) and func.attr == "join":
                return (
                    "shlex_join_shell",
                    "medium",
                    "subprocess argv built via str.join() — verify inputs are sanitized",
                )
    return None
...
class _CommandInjectionVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        result = _is_shell_command_call(node)
        if result:
...
class CommandInjectionAnalyzer:
    """Detect dynamic shell command construction in Python projects.

    Flags os.system/popen and subprocess calls where commands are built
    ...
    def summary(self) -> str:
        ...
        stats = self.stats
        lines = [
            f"Command injection risks: {stats.total_findings} findings in "
    ...
    def to_context(self, limit: int = 30) -> str:
        ...
        if not self._findings:
            lines.append("No dynamic shell command construction found.")
        else:
