...
class DevProgram:
    ...
    def from_yaml(cls, text: str, assistant: CodeAssistant) -> DevProgram:
        """Load a program from YAML (requires ``pyyaml``)."""
        try:
            import yaml
        except ImportError as exc:
            raise ImportError(
                "YAML support requires pyyaml. Install with: pip install 'devai[yaml]'"
            ) from exc
        data = yaml.safe_load(text)
        if not isinstance(data, dict):
            raise ValueError("YAML program must be a mapping")
