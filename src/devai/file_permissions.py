...
_WORLD_WRITABLE = frozenset({0o777, 0o666, 0o776, 0o767, 0o766})
_WORLD_READABLE_SENSITIVE = frozenset({0o644, 0o666, 0o777})
_CHMOD_PATTERN = re.compile(r"os\.chmod\s*\([^)]*,\s*(0o?7\d{2}|0o?6\d{2})")
...
def _parse_octal(value: ast.AST) -> int | None:
    if isinstance(value, ast.Constant) and isinstance(value.value, int):
        return value.value
    if isinstance(value, ast.Constant) and isinstance(value.value, str):
        try:
            return int(value.value, 8) if value.value.startswith("0") else int(value.value)
        except ValueError:
            return None
    return None
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(func, ast.Attribute):
            if func.attr == "chmod" and len(node.args) >= 2:
                mode = _parse_octal(node.args[1])
                if mode is not None and (mode & 0o002 or mode in _WORLD_WRITABLE):
                    self.findings.append(
                        FilePermissionFinding(
                            ...
                            severity="high",
                            message=f"os.chmod with mode {oct(mode)} allows world write access",
            ...
            if func.attr == "makedirs":
                for kw in node.keywords:
                    if kw.arg == "mode":
                        mode = _parse_octal(kw.value)
                        if mode is not None and (mode & 0o002 or mode in _WORLD_WRITABLE):
                            self.findings.append(
                                FilePermissionFinding(
                                    path=self.path,
                                    lineno=node.lineno,
                                    pattern="permissive_makedirs",
                                    severity="medium",
                                    message=f"os.makedirs with mode {oct(mode)} may create world-writable directories",
                                    function=self._current_function(),
                                )
                            )
        self.generic_visit(node)
            ...
            if _CHMOD_PATTERN.search(line):
                findings.append(
                    FilePermissionFinding(
                        ...
                        pattern="world_writable_chmod",
                        severity="high",
                        message="os.chmod with permissive mode detected",
                    )
                )
        return findings
