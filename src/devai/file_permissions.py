...
_PERMISSIVE_MODES = frozenset({0o777, 0o666, 0o7777, 0o6666, 777, 666, 511, 438})
_CHMOD_PATTERN = re.compile(r"chmod\s*\(\s*[^,]+,\s*(0o?)?(777|666|511|438)\b")
...
class _FilePermissionVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(func, ast.Attribute) and func.attr == "chmod":
            ...
            if mode_arg is None and node.keywords:
                for kw in node.keywords:
                    if kw.arg == "mode":
                        mode_arg = kw.value
                        break
            if isinstance(mode_arg, ast.Constant) and mode_arg.value in _PERMISSIVE_MODES:
                self.findings.append(
                    FilePermissionFinding(
                        ...
                        severity="medium",
                        message=f"Overly permissive chmod mode {mode_arg.value}",
...
class FilePermissionAnalyzer:
    """Detect chmod and file writes with world-readable or world-writable permissions."""
            ...
            if _CHMOD_PATTERN.search(line):
                findings.append(
                    FilePermissionFinding(
                        ...
                        pattern="permissive_chmod",
                        severity="medium",
                        message="Overly permissive chmod mode detected in source",
                    )
                )
        return findings
    ...
    def to_context(self, limit: int = 30) -> str:
        ...
        if not self._findings:
            lines.append("No overly permissive file permissions found.")
        else:
