...
class LitestarAnalyzer:
    ...
    Scans Litestar entry files, route handlers, and config for hardcoded secrets,
    open CORS, disabled CSRF, exposed OpenAPI docs, SSRF targets, and unprotected routes.
    """
    ...
    def _scan_line(
        ...
        if "CORSConfig" in stripped or "allow_origins" in stripped:
            info.has_cors = True
        if any(k in stripped for k in ("JWTAuth", "OAuth2PasswordBearer", "guards=", "requires_auth")):
            info.has_auth = True
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (CSRF_DISABLED_PATTERN, "csrf_disabled", "high",
             "CSRF protection disabled or wildcard exclude — enable CSRFConfig"),
            (OPENAPI_EXPOSED_PATTERN, "openapi_exposed", "medium",
             "OpenAPI docs exposed — disable or protect in production"),
