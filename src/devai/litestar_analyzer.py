...
DOCS_EXPOSED_PATTERN = re.compile(
    r"(?:path\s*=\s*['\"]/(?:docs|schema|openapi)|"
    r"openapi_config\s*=\s*OpenAPIConfig\s*\(|"
    r"docs_url\s*=\s*['\"]/)",
    re.IGNORECASE,
)
...
SESSION_SECRET_HARDCODED_PATTERN = re.compile(
    r"SessionMiddleware\s*\([^)]*secret\s*=\s*['\"][^'\"${}]+['\"]",
    re.IGNORECASE | re.DOTALL,
)
...
class LitestarAnalyzer:
    ...
    Scans Litestar entry files, route handlers, and middleware for hardcoded
    secrets, open CORS, exposed OpenAPI docs, disabled CSRF, insecure session
    cookies, SSRF targets, and unprotected admin routes.
    """
    ...
    def _scan_line(
        ...
        if "CORSConfig" in stripped or "allow_origins" in stripped:
            info.has_cors = True
        if any(k in stripped for k in ("JWTAuth", "OAuth2PasswordBearer", "AuthorizationMiddleware")):
            info.has_auth = True
        if "CSRFConfig" in stripped or "csrf_config" in stripped:
            info.has_csrf = True
        if "AllowedHostsMiddleware" in stripped or "allowed_hosts" in stripped:
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (CSRF_DISABLED_PATTERN, "csrf_disabled", "high",
             "CSRF protection disabled or wildcard exclude — enable CSRF for state-changing routes"),
            ...
            (TRUSTED_HOST_DISABLED_PATTERN, "trusted_host_wildcard", "medium",
             "AllowedHostsMiddleware allows all hosts — restrict allowed_hosts"),
            (SESSION_SECRET_HARDCODED_PATTERN, "session_secret_hardcoded", "high",
             "hardcoded session secret in SessionMiddleware — load from environment variables"),
        ]
            ...
            if not any(f.kind == "session_secret_hardcoded" for f in findings):
                findings.append(
                    LitestarFinding(
                        ...
                        severity="high",
                        message="hardcoded session secret in SessionMiddleware — load from environment variables",
                        path=rel,
                        lineno=1,
                        line="",
                    )
