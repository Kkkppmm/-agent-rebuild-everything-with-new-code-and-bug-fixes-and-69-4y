...
LITESTAR_ROUTE_PATTERN = re.compile(
    r"(?:@(?:get|post|put|delete|patch|head|options|route)\b|Router\s*\()",
    re.IGNORECASE,
)
...
CSRF_DISABLED_PATTERN = re.compile(
    r"CSRFConfig\s*\([^)]*enabled\s*=\s*False",
    re.IGNORECASE | re.DOTALL,
)
OPENAPI_EXPOSED_PATTERN = re.compile(
    r"(?:OpenAPIConfig|openapi_config)\s*\([^)]*(?:path|title)\s*=\s*['\"]/",
    re.IGNORECASE | re.DOTALL,
)
...
class LitestarAnalyzer:
    ...
    Scans Litestar entry files, routes, and middleware for hardcoded session
    secrets, disabled CSRF, open CORS, exposed OpenAPI docs, debug/reload mode,
    ...
    def _scan_line(
        ...
        for route_kw in ("@get", "@post", "@put", "@delete", "@patch", "@route", "Router("):
        ...
        if "AllowedHostsConfig" in stripped or "allowed_hosts" in stripped:
            info.has_trusted_host = True
        if "SessionConfig" in stripped or "SessionMiddleware" in stripped:
        ...
        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            ...
            (SESSION_SECRET_HARDCODED_PATTERN, "session_secret_hardcoded", "high",
             "hardcoded SessionConfig secret — use environment variables"),
            ...
            (CSRF_DISABLED_PATTERN, "csrf_disabled", "high",
             "CSRF protection disabled — enable CSRFConfig in production"),
            (OPENAPI_EXPOSED_PATTERN, "openapi_exposed", "medium",
             "OpenAPI docs exposed at root path — restrict in production"),
