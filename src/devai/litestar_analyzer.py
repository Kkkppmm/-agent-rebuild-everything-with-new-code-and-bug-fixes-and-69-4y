...
SESSION_SECRET_HARDCODED_PATTERN = re.compile(
    r"(?:ServerSideSessionConfig|CookieBackendConfig|ClientSideSessionConfig)\s*\([^)]*"
...
CORS_WILDCARD_PATTERN = re.compile(
    r"allow_origins\s*=\s*\[[^\]]*['\"]\*['\"]|"
    r"CORSConfig\([^)]*allow_origins\s*=\s*\[[^\]]*['\"]\*['\"]",
    re.IGNORECASE | re.DOTALL,
)
...
CSRF_DISABLED_PATTERN = re.compile(
    r"CSRFConfig\([^)]*(?:exclude\s*=\s*\[[^\]]*['\"]\*['\"]|enabled\s*=\s*False)",
    re.IGNORECASE | re.DOTALL,
)
OPENAPI_EXPOSED_PATTERN = re.compile(
    r"(?:OpenAPIConfig\([^)]*(?:path|root_schema_site)|"
    r"openapi_config\s*=\s*OpenAPIConfig|"
    r"root_schema_site\s*=\s*['\"]swagger['\"]|"
    r"path\s*=\s*['\"]/(?:schema|docs|openapi))",
    re.IGNORECASE,
)
...
TRUSTED_HOST_DISABLED_PATTERN = re.compile(
    r"allowed_hosts\s*=\s*\[[^\]]*['\"]\*['\"]|"
    r"AllowedHostsConfig\([^)]*allowed_hosts\s*=\s*\[[^\]]*['\"]\*['\"]",
    re.IGNORECASE | re.DOTALL,
)
...
TEMPLATE_AUTOESCAPE_OFF_PATTERN = re.compile(
...
class LitestarAnalyzer:
    ...
    Scans Litestar entry files, controllers, and configs for hardcoded secrets,
    open CORS, disabled CSRF, exposed OpenAPI docs, debug mode, unsafe static
    file mounts, missing trusted hosts, SSRF targets, and unprotected admin routes.
    """
    ...
    def _scan_line(
        ...
        for decorator in ("get", "post", "put", "delete", "patch", "route"):
        ...
        if "CORSConfig" in stripped or "allow_origins" in stripped:
            info.has_cors = True
        if "CSRFConfig" in stripped:
            info.has_csrf = True
        if any(k in stripped for k in ("Guard", "requires=", "Authorization", "jwt_auth")):
            info.has_auth = True
        if "HTTPSRedirectMiddleware" in stripped or "HTTPSRedirectConfig" in stripped:
            info.has_https_redirect = True
        if "AllowedHostsConfig" in stripped or "TrustedHostMiddleware" in stripped:
            info.has_trusted_host = True

        checks: list[tuple[re.Pattern[str], str, str, str]] = [
            (HARDCODED_SECRET_PATTERN, "hardcoded_secret", "high",
             "hardcoded secret in Litestar app — use environment variables or pydantic-settings"),
            ...
            (CSRF_DISABLED_PATTERN, "csrf_disabled", "high",
             "CSRF protection disabled or wildcard exempt — enable CSRF for state-changing routes"),
            ...
            (DANGEROUS_ROUTE_PATTERN, "dangerous_route", "high",
             "admin/debug/internal route — ensure authentication guards are applied"),
            (STATIC_FILES_UNSAFE_PATTERN, "static_files_unsafe", "high",
             "StaticFilesConfig serves filesystem root — restrict directory scope"),
            (TRUSTED_HOST_DISABLED_PATTERN, "trusted_host_wildcard", "medium",
             "AllowedHostsConfig allows all hosts — restrict allowed_hosts"),
            ...
            (TEMPLATE_AUTOESCAPE_OFF_PATTERN, "template_autoescape_off", "medium",
             "template autoescape disabled — XSS risk in rendered templates"),
        ]
