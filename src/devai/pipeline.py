...
class DevPipeline:
    ...
    def review(self, code: str) -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.CODE_REVIEW)
    ...
    def debug(self, code: str, error: str) -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.DEBUG)
        output = chain.run(error=error, code=code)
        self.results.append(PipelineResult(step="debug", output=output))
        return output

    def generate_tests(self, code: str, framework: str = "pytest") -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.TEST_GEN)
    ...
    def security_review(self, code: str) -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.SECURITY_REVIEW)
        output = chain.run(language=self.language, code=code)
    ...
    def refactor(self, code: str, goals: str = "readability and performance") -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.REFACTOR)
    ...
    def explain(self, code: str) -> str:
        chain = SimpleChain(client=self.client, prompt=dev_prompts.EXPLAIN_CODE)
        output = chain.run(language=self.language, code=code)
    ...
    def run_all(self, code: str, steps: list[PipelineStep] | None = None) -> dict[str, str]:
