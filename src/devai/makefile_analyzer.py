...
class MakefileAnalyzer:
    ...
    Scans for curl-pipe-to-shell, destructive rm -rf, sudo usage, secrets in
    variables, chmod 777, git force-push, and eval usage.
    """
            ...
            if RM_RF_ROOT_PATTERN.search(line):
                findings.append(
                    MakefileFinding(
                        kind="destructive_rm",
                        severity="high",
                        message="rm -rf on root or home directory is dangerous",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if EVAL_PATTERN.search(line):
                findings.append(
                    MakefileFinding(
                        kind="eval_usage",
                        severity="medium",
                        message="eval in Makefile recipes can execute arbitrary code",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
