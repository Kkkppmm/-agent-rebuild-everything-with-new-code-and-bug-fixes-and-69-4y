...
class ProjectDetector:
    ...
    def detect(self, path: str | Path = ".") -> ProjectProfile:
        ...
        if _exists(root, ".envrc") or _exists(root, "direnv.toml"):
            package_managers.append("direnv")
        if _exists(root, "buf.yaml") or _exists(root, "buf.gen.yaml"):
            package_managers.append("buf")
        if _exists(root, "openapi.yaml") or _exists(root, "openapi.json") or _exists(root, "swagger.yaml"):
