...
DISABLED_SECURITY_RULE_PATTERN = re.compile(
    r"(?:ignore|extend-ignore|per-file-ignores)\s*=\s*[^\n]*[\"']?S\d{3}[\"']?",
    re.IGNORECASE,
)
...
PER_FILE_SECURITY_IGNORE_PATTERN = re.compile(
    r"(?:per-file-ignores|settings\.py|config\.py|secrets?\.py|auth\.py)[^\n]*S\d{3}",
    re.IGNORECASE,
)
DOCTEST_IGNORE_ALL_PATTERN = re.compile(
    r"doctest-ignore-import-errors\s*=\s*true\b",
    re.IGNORECASE,
)
...
class Flake8Analyzer:
    ...
    Scans .flake8, setup.cfg, tox.ini, and pyproject.toml [tool.flake8] for
    broad ignore rules, disabled S (bandit) rules, source exclusions, and
    hardcoded secrets.
    """
    ...
    def config_files(self) -> list[Path]:
        ...
        for name in CONFIG_NAMES:
            ...
            if name == ".flake8":
                found.append(path)
                continue
            if name == "setup.cfg" and not SETUP_CFG_FLAKE8_SECTION.search(text):
                continue
            if name == "tox.ini" and "[flake8" not in text.lower():
                continue
            if name == "pyproject.toml" and "[tool.flake8" not in text and "[tool:flake8" not in text:
                continue
            found.append(path)
        return found
    ...
    def _scan_line(
        ...
        if IGNORE_ALL_PATTERN.search(line):
            findings.append(
                Flake8Finding(
                    kind="ignore_broad",
                    severity="high",
                    message="broad ignore rules disable entire E/W/F categories — narrow ignores",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
            ...
            findings.append(
                Flake8Finding(
                    kind="ignore_category",
                    severity="medium",
                    message="ignore disables an entire error category — prefer specific codes",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if DISABLED_SECURITY_RULE_PATTERN.search(line):
            findings.append(
                Flake8Finding(
                    ...
                    severity="high",
                    message="Flake8 S (bandit) rule disabled — re-enable security linting",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
        ...
        if DOCTEST_IGNORE_ALL_PATTERN.search(line):
            findings.append(
                Flake8Finding(
                    kind="doctest_ignore_import_errors",
                    severity="low",
                    message="doctest-ignore-import-errors=true hides doctest import failures",
                    path=rel,
                    lineno=lineno,
                    line=line,
                )
    ...
    def _analyze_ini_file(self, path: Path) -> tuple[list[Flake8Finding], Flake8Info]:
        findings: list[Flake8Finding] = []
    ...
    def _analyze_toml_file(self, path: Path) -> tuple[list[Flake8Finding], Flake8Info]:
        findings: list[Flake8Finding] = []
        rel = str(path.relative_to(self.root))
        try:
            raw_lines = path.read_text(encoding="utf-8", errors="replace").splitlines()
    ...
    def _analyze_file(self, path: Path) -> tuple[list[Flake8Finding], Flake8Info]:
        ...
        return self._analyze_ini_file(path)
