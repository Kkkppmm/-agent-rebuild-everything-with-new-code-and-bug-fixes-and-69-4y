...
class CircleCIAnalyzer:
    ...
    Scans for unpinned orbs, secrets in environment blocks, privileged Docker
    settings, inline SSH keys, and curl-pipe-to-shell patterns in run steps.
    """
    ...
    def _analyze_file(self, path: Path) -> tuple[list[CircleCIFinding], CircleCIInfo]:
        ...
        for lineno, raw in enumerate(raw_lines, start=1):
            ...
            if line.startswith("orbs:") or re.match(r"^\s+\w+:\s*$", raw) and "orb" in line.lower():
                orb_match = re.match(r"^\s*(\S+):\s*$", line)
                if orb_match and not line.startswith("jobs"):
                    info.orbs.append(orb_match.group(1))

            if in_jobs and indent == 2 and line.endswith(":") and not line.startswith("-"):
                job_name = line[:-1].strip()
                if job_name not in ("docker", "machine", "resource_class", "steps", "environment"):
            ...
            if UNPINNED_ORB_PATTERN.search(line) and ("orb" in line.lower() or "@" in line):
                if not re.search(r"@v?\d+\.\d+", line):
                    findings.append(
                        CircleCIFinding(
                            kind="unpinned_orb",
                            severity="high",
                            message="orb pinned to mutable branch (@main/@latest) — pin to a version tag",
                            path=rel,
                            lineno=lineno,
                            line=raw.strip(),
                        )
            ...
            if PRIVILEGED_DOCKER_PATTERN.search(line):
                findings.append(
                    CircleCIFinding(
                        ...
                        severity="high",
                        message="privileged: true grants full host access in Docker executor",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if SSH_KEY_INLINE_PATTERN.search(line):
                findings.append(
                    CircleCIFinding(
                        kind="inline_ssh_key",
                        severity="high",
                        message="inline SSH key detected — use CircleCI SSH keys or contexts",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
