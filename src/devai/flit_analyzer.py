...
FLIT_PYPROJECT_NAMES = ("pyproject.toml",)
FLIT_INI_NAMES = ("flit.ini",)
FLIT_MARKER_PATTERN = re.compile(
    r"(?:^\[tool\.flit\]|^\[tool\.flit\.|flit_core\.buildapi|flit\.buildapi|"
    r"^\[tool\.flit\.metadata\]|^\[tool\.flit\.sdist\])",
    re.IGNORECASE | re.MULTILINE,
)
FLIT_INI_MARKER_PATTERN = re.compile(
    r"(?:^\[metadata\]|^\[scripts\]|^\[sdist\]|module\s*=)",
    re.IGNORECASE | re.MULTILINE,
)
...
INDEX_URL_PATTERN = re.compile(
    r"(?:index-url|extra-index-url|url|repo|repository)\s*=\s*[\"']?(\S+)[\"']?",
    re.IGNORECASE,
)
...
INSECURE_ENV_VAR_PATTERN = re.compile(
    r"(?:FLIT_PASSWORD|FLIT_USERNAME|TWINE_PASSWORD|TWINE_USERNAME|"
    r"PYPI_PASSWORD|PYPI_TOKEN)\s*=\s*[\"'][^\"']+[\"']",
    re.IGNORECASE,
)
DANGEROUS_SCRIPT_PATTERN = re.compile(
    r"(?:^\[scripts\]|post-install|pre-install|entry-points)",
    re.IGNORECASE,
)
UNSAFE_MODULE_PATTERN = re.compile(
    r"module\s*=\s*[\"'](?:\.\.|/|\\)",
    re.IGNORECASE,
)
...
def _is_flit_file(path: Path) -> bool:
    ...
    name = path.name
    if name in FLIT_INI_NAMES:
        return True
    if name in FLIT_PYPROJECT_NAMES:
        try:
...
class FlitAnalyzer:
    ...
    Scans pyproject.toml (with [tool.flit]), flit.ini, and related files
    ...
    curl-pipe-to-shell in scripts, disabled SSL verification, unsafe module
    paths, and publish credentials in plain text.
    """
    ...
    def _analyze_file(self, path: Path) -> tuple[list[FlitFinding], FlitInfo]:
        ...
        for lineno, line in enumerate(raw_lines, start=1):
            ...
            module_match = re.search(r"module\s*=\s*[\"']([^\"']+)[\"']", stripped, re.IGNORECASE)
            if module_match:
            ...
            if requires_match:
                for dep in re.findall(r"[\"']([^\"']+)[\"']", requires_match.group(1)):
                    info.dependencies.append(dep.split(";")[0].strip())
            ...
            if dep_match and not stripped.startswith("["):
                ...
                if dep_name not in ("name", "version", "description", "authors", "license", "module"):
            ...
            if HARDCODED_SECRET_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        ...
                        severity="high",
                        message="hardcoded secret in Flit config — use FLIT_PASSWORD or CI secret stores",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if PYPI_TOKEN_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        kind="pypi_token",
                        severity="high",
                        message="PyPI token in Flit config — use keyring or FLIT_PASSWORD env var",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if INSECURE_ENV_VAR_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        ...
                        severity="high",
                        message="publish credentials in Flit config — use env vars or keyring",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if INSECURE_HTTP_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        kind="insecure_http",
                        severity="medium",
                        message="insecure HTTP URL — use HTTPS for PyPI indexes and project home pages",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if (
                ...
                and not re.match(r"requires\s*=\s*\[", stripped, re.IGNORECASE)
                and (
                    re.search(r"(?:requires|dependencies|dev-dependencies)", stripped, re.IGNORECASE)
            ...
            if CURL_PIPE_SHELL_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        ...
                        severity="high",
                        message="curl/wget piped to shell in Flit config — vendor scripts with checksum verification",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if UNSAFE_MODULE_PATTERN.search(line):
                findings.append(
                    FlitFinding(
                        kind="unsafe_module",
                        severity="high",
                        message="unsafe module path — module must not reference parent or absolute paths",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
            ...
            if DANGEROUS_SCRIPT_PATTERN.search(line) and "curl" in line.lower():
                findings.append(
                    FlitFinding(
                        kind="dangerous_script",
                        severity="medium",
                        message="script hook in Flit config — review for supply-chain risks",
                        path=rel,
                        lineno=lineno,
                        line=line,
                    )
    ...
    def generate_hardened_config(self) -> str:
        """Scaffold a hardened pyproject.toml snippet with secure Flit defaults."""
        return """\
# pyproject.toml — hardened Flit defaults
[build-system]
...
dependencies = [
    "requests==2.31.0",  # pin dependencies for reproducible builds
]

[tool.flit.metadata]
module = "my_package"
# Use HTTPS for home-page; never commit PyPI tokens here
# Store publish credentials via:
#   export FLIT_USERNAME=__token__
#   export FLIT_PASSWORD=pypi-<token>
"""
    ...
    def to_context(self) -> str:
        ...
        for info in self.infos:
            ...
            lines.append(
                ...
                f"module={module}, {len(info.dependencies)} dependency(ies)"
            )
