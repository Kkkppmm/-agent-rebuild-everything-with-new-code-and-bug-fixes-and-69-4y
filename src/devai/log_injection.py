...
_USER_INPUT_RE = re.compile(
    r"(request|user|input|param|query|header|cookie|remote|client|ip|addr|"
    r"username|email|message|body|payload|data|value)",
    re.IGNORECASE,
)

_LOG_LEVELS = frozenset({"debug", "info", "warning", "warn", "error", "critical", "exception"})
_LOGGER_METHODS = frozenset({"log"})
...
def _is_log_call(node: ast.Call) -> bool:
    if not isinstance(node.func, ast.Attribute):
        return False
    attr = node.func.attr
    if attr in _LOG_LEVELS or attr in _LOGGER_METHODS:
        return True
    if attr == "write" and isinstance(node.func.value, ast.Attribute):
        if node.func.value.attr in {"stdout", "stderr"}:
            return True
    return False
        ...
        if _is_log_call(node) and node.args:
            ...
            if _is_dynamic_string(msg_arg):
                self._add(
                    ...
                    "dynamic_log_message",
                    "medium",
                    "Log message built from user input — use structured logging with extra fields",
                )
            elif len(node.args) > 1 and _looks_like_user_input(node.args[1]):
                self._add(
                    node,
                    "log_format_injection",
                    "medium",
                    "User input passed as log format argument — risk of format string injection",
                )

        name = _call_name(node)
        if name == "print" and node.args and _is_dynamic_string(node.args[0]):
            self._add(
                node,
                "print_user_input",
                "low",
                "Printing user-controlled data — may leak sensitive info to stdout",
            )
...
class LogInjectionAnalyzer:
    """Detect dynamic log message construction that may enable log injection.

    Flags logger calls where messages are built from request parameters,
    user input, or other untrusted data without sanitization.
    """
    ...
    def by_pattern(self, pattern: str) -> list[LogInjectionFinding]:
