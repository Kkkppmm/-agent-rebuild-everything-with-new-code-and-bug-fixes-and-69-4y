...
class DevAIConfig:
    ...
    def from_env(cls, **overrides: Any) -> DevAIConfig:
        ...
        Reads ``DEVAI_PROVIDER`` (default: ``openai``), ``DEVAI_API_KEY``,
        ``OPENAI_API_KEY`` (fallback), ``DEVAI_MODEL``, ``DEVAI_BASE_URL``,
        and other ``DEVAI_*`` settings. Set ``DEVAI_MOCK=1`` for mock mode.
        """
        if os.environ.get("DEVAI_MOCK", "").lower() in {"1", "true", "yes"}:
            return cls(api_key="mock", model=overrides.pop("model", None) or "mock-model", **overrides)

        provider = os.environ.get("DEVAI_PROVIDER", "openai")
        api_key = overrides.pop("api_key", None)
        if api_key is None:
            api_key = os.environ.get("DEVAI_API_KEY") or os.environ.get("OPENAI_API_KEY")
        model = overrides.pop("model", None) or os.environ.get("DEVAI_MODEL")
        return cls.from_provider(provider, model=model, api_key=api_key, **overrides)
