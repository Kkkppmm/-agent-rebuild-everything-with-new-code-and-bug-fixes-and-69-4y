...
class DevAIConfig:
    ...
    def from_env(cls, **overrides: Any) -> DevAIConfig:
        ...
        Reads ``DEVAI_PROVIDER`` (openai, ollama, mock), ``DEVAI_API_KEY``,
        ``DEVAI_BASE_URL``, ``DEVAI_MODEL``, ``DEVAI_MAX_TOKENS``, and
        ``DEVAI_TEMPERATURE``. Set ``DEVAI_USE_MOCK=1`` to force the mock
        provider. Keyword arguments override environment values.
        """
        use_mock = overrides.pop("use_mock", None)
        if use_mock is None:
            use_mock = os.environ.get("DEVAI_USE_MOCK", "").lower() in (
                "1",
                "true",
                "yes",
            )
        if use_mock:
            return cls.from_provider("mock", **overrides)

        provider = overrides.pop("provider", None) or os.environ.get(
            "DEVAI_PROVIDER", "openai"
        )
        return cls.from_provider(provider, **overrides)
