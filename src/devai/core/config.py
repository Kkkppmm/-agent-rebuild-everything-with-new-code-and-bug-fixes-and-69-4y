...
class DevAIConfig:
    ...
    def from_env(cls, **overrides: Any) -> DevAIConfig:
        ...
        Reads ``DEVAI_PROVIDER`` (``openai``, ``ollama``, or ``mock``), plus
        ...
        if provider == "mock":
            return cls(
                api_key="mock",
                model=os.environ.get("DEVAI_MODEL", "mock-model"),
                **overrides,
            )
        if provider == "ollama":
            return cls.for_ollama(
                model=os.environ.get("DEVAI_MODEL", "llama3.2"),
                base_url=os.environ.get("DEVAI_BASE_URL", "http://localhost:11434/v1"),
                **overrides,
            )
