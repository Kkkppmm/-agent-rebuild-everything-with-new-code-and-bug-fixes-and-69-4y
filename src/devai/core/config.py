...
class DevAIConfig:
    ...
    def from_env(cls, **overrides: Any) -> DevAIConfig:
        ...
        ``DEVAI_MAX_TOKENS``, ``DEVAI_TEMPERATURE``, and ``DEVAI_PROVIDER``.
        ...
        if provider.lower() == "ollama":
            return cls.for_ollama(
                model=overrides.pop("model", None) or os.environ.get("DEVAI_MODEL", "llama3.2"),
                base_url=overrides.pop("base_url", None)
                or os.environ.get("DEVAI_BASE_URL", "http://localhost:11434/v1"),
                **overrides,
            )
        return cls(
            api_key=overrides.pop("api_key", None),
            model=overrides.pop("model", None) or os.environ.get("DEVAI_MODEL", "gpt-4o-mini"),
            base_url=overrides.pop("base_url", None)
            or os.environ.get("DEVAI_BASE_URL", "https://api.openai.com/v1"),
            **overrides,
        )
