...
CREDENTIAL_INLINE_PATTERN = re.compile(
...
DISABLE_SECURITY_PATTERN = re.compile(
    r"disableSecurity\s*\(\s*\)|setSecurityRealm\s*\(\s*null\s*\)",
    re.IGNORECASE,
)
UNSAFE_SH_INTERPOLATION_PATTERN = re.compile(
    r"sh\s+['\"].*\$\{.*\}.*['\"]",
    re.IGNORECASE,
)
SSH_KEY_INLINE_PATTERN = re.compile(
    r"(ssh-rsa|ssh-ed25519|BEGIN (RSA |OPENSSH )?PRIVATE KEY)",
    re.IGNORECASE,
)
MASTER_BRANCH_ONLY_PATTERN = re.compile(
    r"branch\s*==\s*['\"]master['\"]|branch\s*==\s*['\"]main['\"]",
    re.IGNORECASE,
)
...
def _is_jenkinsfile(path: Path) -> bool:
    ...
    if name in JENKINSFILE_NAMES:
        return True
    if name.lower().endswith(".jenkinsfile"):
        return True
    return False
...
class JenkinsfileAnalyzer:
    ...
    Scans for inline credentials, disableSecurity calls, unsafe shell
    interpolation, inline SSH keys, and curl-pipe-to-shell patterns.
    """
    ...
    def _analyze_file(self, path: Path) -> tuple[list[JenkinsfileFinding], JenkinsfileInfo]:
        ...
        for lineno, raw in enumerate(raw_lines, start=1):
            ...
            if "docker" in line.lower() and ("image" in line.lower() or "agent" in line.lower()):
                info.uses_docker = True

            if CREDENTIAL_INLINE_PATTERN.search(line):
                findings.append(
                    JenkinsfileFinding(
                        kind="inline_credential",
                        severity="high",
                        message="inline credential detected — use Jenkins credentials binding",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if DISABLE_SECURITY_PATTERN.search(line):
                findings.append(
                    JenkinsfileFinding(
                        kind="disable_security",
                        severity="high",
                        message="disableSecurity or null SecurityRealm disables Jenkins authentication",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if UNSAFE_SH_INTERPOLATION_PATTERN.search(line):
                findings.append(
                    JenkinsfileFinding(
                        kind="unsafe_sh_interpolation",
                        severity="high",
                        message="shell step interpolates variables — use withCredentials and quoting",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if SSH_KEY_INLINE_PATTERN.search(line):
                findings.append(
                    JenkinsfileFinding(
                        kind="inline_ssh_key",
                        severity="high",
                        message="inline SSH key detected — use Jenkins SSH credentials",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
            ...
            if MASTER_BRANCH_ONLY_PATTERN.search(line) and "when" in line.lower():
                findings.append(
                    JenkinsfileFinding(
                        kind="branch_gate",
                        severity="low",
                        message="branch gate on main/master — ensure PR builds run security checks too",
                        path=rel,
                        lineno=lineno,
                        line=raw.strip(),
                    )
...
"""
    ...
    def to_context(self) -> str:
        ...
        for info in self.infos:
            ...
            lines.append(f"  - {info.path}: {len(info.stages)} stage(s), stages=[{stages}]")
