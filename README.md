...
- **TypingCoverage** — Measure type hint coverage across functions and methods
- **DocstringCoverage** — Measure docstring coverage for modules, classes, and functions
- **DependencyParser** — Parse and analyze dependencies from requirements.txt and pyproject.toml
- **DevTools** — Unified facade for imports, secrets, typing, docstrings, deps, and changelog analysis
...
```bash
devai init                       # scaffold config, programs, schedule, and starter script
devai config-init                # creates .devai.yaml only
devai config-show                # show resolved settings
```

```python
from devai import DevAI, scaffold_project

# Scaffold a new project directory
result = DevAI.scaffold("./my-ai-app", provider="openai", model="gpt-4o-mini")
print(result.created)  # [.devai.yaml, programs/pre-commit.yaml, ...]
```
