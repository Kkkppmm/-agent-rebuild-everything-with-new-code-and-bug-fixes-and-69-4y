...
- **SecurityScanner** — Unified static security scan combining secrets, injections, dangerous calls, insecure random, weak crypto, log injection, SSRF, path traversal, clickjacking, host header injection, and session fixation checks
- **ClickjackingAnalyzer** — Detect HTML responses missing X-Frame-Options or CSP frame-ancestors
- **HostHeaderAnalyzer** — Detect host header injection in redirects and URL construction
- **SessionFixationAnalyzer** — Detect session IDs in URL parameters and missing session regeneration on login
