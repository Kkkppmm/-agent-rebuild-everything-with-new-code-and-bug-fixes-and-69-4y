...
- **CibuildwheelAnalyzer** — Audit cibuildwheel pyproject.toml and cibuildwheel.toml for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, sensitive environment-pass variables, trusted-host bypasses, dangerous before-build/before-all hooks, and curl-pipe-to-shell in wheel build scripts
