...
- **MakefileAnalyzer** — Audit Makefiles for curl-pipe-to-shell, dangerous `rm -rf`, hardcoded secrets, sudo usage, and unpinned pip installs
- **KubernetesAnalyzer** — Audit K8s manifests for privileged mode, host namespaces, root execution, secrets in env, and missing resource limits
