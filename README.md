...
- **VitestAnalyzer** — Audit vitest.config.*, vitest.workspace.*, and vite test blocks for hardcoded secrets, disabled server.fs.strict sandboxing, remote setupFiles URLs, exposed browser/server hosts, sensitive test env vars, and insecure HTTP proxies
- **JestAnalyzer** — Audit jest.config.* and package.json jest blocks for hardcoded secrets, remote setup/globalSetup URLs, insecure testURL values, sensitive globals, path traversal moduleNameMapper, and symlink-enabled haste maps
