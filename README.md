...
- **CibuildwheelAnalyzer** — Audit cibuildwheel pyproject.toml, cibuildwheel.toml, and setup.cfg for hardcoded secrets, insecure HTTP URLs, privileged Docker mode, unpinned manylinux images, skipped tests, dangerous test commands, and environment variable injection
- **MaturinAnalyzer** — Audit maturin pyproject.toml and Cargo.toml for hardcoded PyPI tokens, insecure HTTP repository URLs, credentials in git/source URLs, unpinned git dependencies, disabled abi3, and unsafe python-source paths
- **ScikitBuildAnalyzer** — Audit scikit-build-core pyproject.toml and CMakeLists.txt for hardcoded secrets, insecure HTTP FetchContent URLs, disabled TLS verification, unpinned git dependencies, and dangerous execute_process commands
