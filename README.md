...
- **HadolintAnalyzer** — Audit `.hadolint.yaml` configs for ignored root-user/latest-tag/secret rules, permissive failure thresholds, wildcard ignores, insecure trusted registries, and hardcoded secrets
- **ActionlintAnalyzer** — Audit `.actionlint.yaml` configs for ignored security rules, self-hosted-runner allowlists, blanket path ignores on workflow YAML, and hardcoded secrets
- **TflintAnalyzer** — Audit `.tflint.hcl` configs for disabled security rules, disabled_by_default, unpinned plugins, force=true upgrades, broad ignore_module blocks, and hardcoded secrets
