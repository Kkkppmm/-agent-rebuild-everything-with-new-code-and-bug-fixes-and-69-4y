...
- **MavenAnalyzer** — Audit Maven pom.xml, settings.xml, and .mvn configs for hardcoded secrets, plain-text server credentials, dynamic dependency versions, insecure HTTP repositories, distributionManagement over HTTP, curl-pipe-to-shell in exec-maven-plugin, and disabled SSL wagon settings
