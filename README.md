...
- **SetuptoolsAnalyzer** — Audit setup.py, setup.cfg, and setuptools pyproject.toml for hardcoded secrets, insecure HTTP index URLs, credentials in VCS URLs, unpinned git dependencies, and loose version constraints
- **HatchAnalyzer** — Audit Hatch pyproject.toml and hatch.toml for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned git dependencies, and loose version constraints
- **FlitAnalyzer** — Audit Flit pyproject.toml and flit.ini for hardcoded secrets, insecure HTTP URLs, credentials in git/source URLs, and unpinned dependencies
- **PdmAnalyzer** — Audit PDM pyproject.toml, .pdm.toml, and pdm.lock for hardcoded PyPI tokens, insecure HTTP indexes, credentials in git/source URLs, unpinned git dependencies, and missing pdm.lock
- **PipfileAnalyzer** — Audit Pipenv Pipfile and Pipfile.lock for hardcoded secrets, insecure HTTP sources, credentials in git/source URLs, unpinned git dependencies, and missing Pipfile.lock
- **CondaAnalyzer** — Audit Conda environment.yml and recipe meta.yaml for hardcoded secrets, insecure HTTP channels, credentials in git/source URLs, and unpinned dependencies
- **RyeAnalyzer** — Audit Rye pyproject.toml, rye.lock, and requirements.lock for hardcoded secrets, insecure HTTP indexes, unpinned git dependencies, and missing lockfiles
- **PipToolsAnalyzer** — Audit pip-tools requirements.in and compiled requirements for hardcoded secrets, insecure HTTP index URLs, credentials in VCS URLs, and unpinned dependencies
