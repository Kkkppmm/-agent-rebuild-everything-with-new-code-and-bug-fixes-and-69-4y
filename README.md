...
- **PipfileAnalyzer** — Audit Pipenv Pipfile and Pipfile.lock for hardcoded PyPI tokens, insecure HTTP sources, credentials in git/source URLs, unpinned git dependencies, loose version constraints, disabled SSL verification, missing Pipfile.lock, and curl-pipe-to-shell in scripts
- **CondaAnalyzer** — Audit Conda environment.yml and recipe meta.yaml for hardcoded Anaconda tokens, insecure HTTP channels, credentials in git URLs, unpinned dependencies, loose version constraints, disabled SSL verification, missing conda-lock, and curl-pipe-to-shell in build scripts
- **HatchAnalyzer** — Audit Hatch pyproject.toml and hatch.toml for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, publish credentials in plain text, trusted-host bypasses, dangerous install scripts, and curl-pipe-to-shell in env hooks
- **FlitAnalyzer** — Audit Flit pyproject.toml and flit.ini for hardcoded PyPI tokens, insecure HTTP home/index URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, unsafe module paths, publish credentials in plain text, trusted-host bypasses, and curl-pipe-to-shell in script hooks
- **PdmAnalyzer** — Audit PDM pyproject.toml, .pdm.toml, and pdm.lock for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, publish credentials in plain text, trusted-host bypasses, missing pdm.lock, dangerous install scripts, and curl-pipe-to-shell in script hooks
...
- **RyeAnalyzer** — Audit Rye pyproject.toml, rye.lock, and requirements.lock for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, trusted-host bypasses, missing rye.lock, unmanaged Python installs, and curl-pipe-to-shell in script hooks
- **PipToolsAnalyzer** — Audit pip-tools requirements.in, compiled requirements.txt, and pyproject.toml [tool.pip-tools] for hardcoded PyPI tokens, insecure HTTP index URLs, credentials in git/source URLs, unpinned compiled dependencies, --allow-unsafe and --emit-index-url flags, trusted-host bypasses, missing compiled lockfiles, and curl-pipe-to-shell in compile scripts
- **SetuptoolsAnalyzer** — Audit setuptools setup.py, setup.cfg, and pyproject.toml for hardcoded PyPI tokens, insecure HTTP index/download URLs, credentials in git/source URLs, unpinned git dependencies, loose version constraints, exec/subprocess in setup.py, deprecated dependency_links, publish credentials in plain text, trusted-host bypasses, unpinned setup_requires, and curl-pipe-to-shell in build scripts
- **MaturinAnalyzer** — Audit maturin pyproject.toml, maturin.toml, and Cargo.toml for hardcoded PyPI/Cargo tokens, insecure HTTP URLs, credentials in git/source URLs, unpinned git dependencies, path traversal in module-name/python-source, sensitive files in include patterns, curl-pipe-to-shell in build hooks, disabled auditwheel stripping, and dangerous repair commands
...
- **YamllintAnalyzer** — Audit `.yamllint` configs for disabled truthy/key-duplicates checks, relaxed extends profiles, broad ignore patterns on CI/deploy YAML, and hardcoded secrets
- **HadolintAnalyzer** — Audit `.hadolint.yaml` configs for ignored security rules, wildcard suppressions, permissive failure thresholds, and hardcoded secrets
...
- **HonoAnalyzer** — Audit Hono apps and `wrangler.toml` for hardcoded auth secrets, open CORS, insecure cookies, internal fetch/proxy targets, unprotected admin routes, and wrangler credential leaks
- **FastifyAnalyzer** — Audit Fastify apps for hardcoded JWT secrets, open CORS, insecure cookies, disabled body limits, internal fetch/proxy targets, unprotected admin routes, and missing security headers
- **ExpressAnalyzer** — Audit Express apps for hardcoded session secrets, open CORS, insecure cookies, disabled body limits, internal fetch/proxy targets, unprotected admin routes, and missing security headers
- **NestJSAnalyzer** — Audit NestJS apps for hardcoded JWT secrets, open CORS, disabled ValidationPipe, TypeORM synchronize, exposed Swagger/GraphQL playgrounds, unprotected admin routes, and SSRF risks
...
- **LangChainAnalyzer** — Audit LangChain agents and RAG chains for hardcoded API keys, dangerous tools (Python REPL, shell), SQL injection in SQL agents, SSRF in web loaders, eval/exec usage, and unrestricted agent execution
...
- **BottleAnalyzer** — Audit Bottle apps for hardcoded secrets, debug/reloader mode, open CORS with credentials, SSTI via template rendering, static file path traversal, shell commands, and SSRF risks
- **PyramidAnalyzer** — Audit Pyramid apps for hardcoded secrets, debug toolbar, disabled CSRF, permissive ACLs, insecure session cookies, SSTI via template rendering, shell commands, and SSRF risks
- **Web2pyAnalyzer** — Audit web2py apps for hardcoded secrets, DAL credential leaks, disabled CSRF, weak auth settings, mass assignment, XSS via BEAUTIFY, shell commands, and SSRF risks
- **RobynAnalyzer** — Audit Robyn apps for hardcoded secrets, debug/reloader mode, open CORS with credentials, bind-all interfaces, reflected user input, shell commands, open redirects, and SSRF risks
...
- **Pep723Analyzer** — Audit PEP 723 inline script metadata blocks for hardcoded secrets, insecure HTTP dependency URLs, credentials in git URLs, unpinned git refs, pip trusted-host bypasses, wildcard versions, and curl-pipe-to-shell patterns
- **TyAnalyzer** — Audit ty.toml and pyproject.toml [tool.ty] for disabled rules, broad import overrides, relaxed analysis settings, insecure extra paths, and hardcoded secrets
- **PyreflyAnalyzer** — Audit pyrefly.toml and pyproject.toml [tool.pyrefly] for disabled presets, broad import overrides, relaxed error settings, insecure paths, and hardcoded secrets
...
- **SemanticReleaseAnalyzer** — Audit python-semantic-release configs for hardcoded tokens, dangerous build commands, path traversal in version paths, no_git_verify, and unsafe tag formats
