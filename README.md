...
- **MaturinAnalyzer** — Audit maturin pyproject.toml, Cargo.toml, and .cargo/config.toml for hardcoded PyPI/cargo registry tokens, insecure HTTP registry URLs, credentials in git/source URLs, unpinned git dependencies, missing Cargo.lock, loose version constraints, [patch] overrides, unsafe RUSTFLAGS, and curl-pipe-to-shell in build scripts
- **CibuildwheelAnalyzer** — Audit cibuildwheel pyproject.toml, cibuildwheel.toml, and .cibuildwheel.toml for hardcoded PyPI/TWINE/CIBW tokens, insecure HTTP build-frontends, credentials in repository URLs, disabled test commands, test-skip wildcards, :latest container images, dangerous shell hooks, and curl-pipe-to-shell in before-all/test-command scripts
