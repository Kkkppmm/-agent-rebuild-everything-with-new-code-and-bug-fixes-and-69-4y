...
- **GatsbyAnalyzer** — Audit `gatsby-config.*` for disabled host checks, plugin credential leaks, pathPrefix traversal, exposed dev servers, internal proxy targets, and production sourcemaps
- **QwikAnalyzer** — Audit `qwik.config.*` for disabled CSRF/origin checks, adapter credential leaks, basePath traversal, exposed dev servers, internal proxy targets, and production sourcemaps
