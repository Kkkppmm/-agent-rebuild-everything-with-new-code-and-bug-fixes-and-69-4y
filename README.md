...
- **QwikAnalyzer** — Audit Qwik City `vite.config.*` and `qwik.config.*` for define/env secret leaks, exposed dev/preview servers, permissive CORS/origin, internal proxies, and disabled CSRF checks
- **GatsbyAnalyzer** — Audit `gatsby-config.*` and `gatsby-node.*` for siteMetadata/plugin secret leaks, insecure siteUrl, internal proxy targets, GraphQL playground exposure, and developMiddleware risks
- **SvelteKitAnalyzer** — Audit `svelte.config.*` for disabled CSRF origin checks, CSP bypass, adapter credential leaks, exposed dev servers, internal proxy targets, and production sourcemaps
