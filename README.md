...
- **OpenRedirectAnalyzer** — Detect open redirect vulnerabilities in HTTP response handlers
- **UnsafeDeserializationAnalyzer** — Detect unsafe deserialization (pickle, yaml.load, marshal)
- **SecurityScanner** — Unified static security scan combining secrets, injections, dangerous calls, insecure random, weak crypto, log injection, SSRF, open redirects, unsafe deserialization, and path traversal checks
...
# Or bootstrap from environment variables (DEVAI_PROVIDER, DEVAI_API_KEY, etc.)
# export DEVAI_PROVIDER=mock
ai = DevAI.from_env()
print(ai.explain("x = 1"))
```
