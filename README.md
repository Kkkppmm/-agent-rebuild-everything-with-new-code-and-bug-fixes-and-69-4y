...
- **CibuildwheelAnalyzer** — Audit cibuildwheel pyproject.toml for hardcoded secrets in CIBW_* environment, insecure HTTP indexes, curl-pipe-to-shell in before/after-build hooks, disabled auditwheel repair, skipped wheel tests, and credentials in git/source URLs
- **MaturinAnalyzer** — Audit maturin pyproject.toml and Cargo.toml for hardcoded PyPI/registry tokens, insecure HTTP registries, secrets in sdist-include, features=all enabling unintended code, unpinned git dependencies, and curl-pipe-to-shell in build scripts
- **ScikitBuildAnalyzer** — Audit scikit-build-core pyproject.toml and CMakeLists.txt for hardcoded secrets in cmake.args, insecure HTTP URLs, disabled TLS verification, unpinned FetchContent dependencies, dangerous execute_process calls, and insecure compile flags
