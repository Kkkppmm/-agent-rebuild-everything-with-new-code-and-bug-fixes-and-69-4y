...
- **Health Checks** — Verify LLM provider connectivity with `check_health()`
- **DevDoctor** — Environment diagnostics for API keys, dependencies, and config files
- **Program Reports** — Export program and workflow results as JSON or Markdown
- **GitContext** — Git-aware context for PR reviews and commit workflows
- **DevTrace** — Lightweight tracing spans for program and workflow steps
- **Template Interpolation** — `${var:}`, `${env:}`, and `${file:}` placeholders
- **Config Files** — Load settings from `.devai.yaml` or `.devai.json`
- **Benchmarking** — Measure LLM latency and throughput
- **Quickstart** — One-line `quickstart()` and `assistant()` helpers
...
```python
from devai import quickstart, check_health, run_doctor

# One-line bootstrap (mock mode for testing without an API key)
