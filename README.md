...
- **CI Reporter** — GitHub Actions annotations and PR comment helpers for automated code gates
...
```python
from devai import CodeAssistant, CIReporter, DevAIConfig

assistant = CodeAssistant(DevAIConfig(api_key="..."))
reporter = CIReporter(assistant)

# Full CI gate (review + security)
report = reporter.full_ci_gate(code=open("app.py").read(), diff=open("changes.diff").read())
print(report.to_github_comment())  # Post as PR comment

# GitHub Actions annotations
for annotation in report.to_actions_annotations():
    print(annotation)
```

```bash
# CLI for CI pipelines
devai ci app.py --gate security --format json
devai ci app.py --diff changes.diff --gate full --format annotations
```
...
devai code-smell path/to/module.py --context "legacy monolith"
devai openapi path/to/routes.py --requirements "OpenAPI 3.1"
devai ci path/to/app.py --gate full --format markdown
