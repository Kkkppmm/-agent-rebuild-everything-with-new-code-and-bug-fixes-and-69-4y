...
- **ReDoSAnalyzer** — Detect regex patterns vulnerable to catastrophic backtracking (nested quantifiers, quantified dot-star)
- **SecurityScanner** — Unified static security scan combining secrets, injections, dangerous calls, insecure random, weak crypto, log injection, SSRF, unsafe deserialization, JWT security, insecure TLS, ReDoS, and path traversal checks
