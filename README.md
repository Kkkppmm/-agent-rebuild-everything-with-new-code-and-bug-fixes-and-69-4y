...
- **GitLabCIAnalyzer** — Audit `.gitlab-ci.yml` for hardcoded secrets, privileged services, unpinned images, and script injection
- **CircleCIAnalyzer** — Audit `.circleci/config.yml` for unpinned orbs/images, privileged containers, embedded SSH keys, and ungated deploys
