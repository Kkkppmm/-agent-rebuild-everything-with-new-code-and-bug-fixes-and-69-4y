...
- **HadolintAnalyzer** — Audit `.hadolint.yaml` configs for ignored security rules (DL3002/DL3007/DL3008), permissive failure thresholds, wildcard trusted registries, and override downgrades
- **NuxtAnalyzer** — Audit `nuxt.config.*` for hardcoded secrets in runtimeConfig, devtools in production, permissive CORS/allowedHosts, disabled SSR/CSRF, and Nitro proxy SSRF risks
