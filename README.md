...
- **PnpmAnalyzer** — Audit pnpm-workspace.yaml, .npmrc, and .pnpmfile.cjs for hardcoded tokens, disabled trust-policy, shamefully-hoist, strict-ssl bypass, insecure registry URLs, and dangerous install hooks
- **BunAnalyzer** — Audit bunfig.toml and bun.lock for hardcoded secrets, insecure registries, global install.scripts, TLS verification bypass, and unpinned git dependencies
- **DenoAnalyzer** — Audit deno.json and deno.jsonc for hardcoded secrets, --allow-all permissions, insecure HTTP imports, unpinned npm:/jsr: specifiers, and dangerous task scripts
