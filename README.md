...
- **StarletteAnalyzer** — Audit Starlette apps for hardcoded session secrets, open CORS with credentials, unsafe StaticFiles mounts, debug mode, missing trusted hosts, and SSRF risks
- **DjangoAnalyzer** — Audit Django projects for SECRET_KEY/DEBUG/CORS/CSRF/SQL/SSRF risks, insecure cookies, and missing security middleware
- **LitestarAnalyzer** — Audit Litestar apps for hardcoded secrets, disabled CSRF, open CORS/OpenAPI docs, debug mode, and SSRF risks
