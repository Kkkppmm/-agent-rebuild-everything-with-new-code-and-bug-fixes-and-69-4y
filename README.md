...
- **MaturinAnalyzer** — Audit maturin pyproject.toml, Cargo.toml, and .cargo/config.toml for hardcoded PyPI/cargo tokens, insecure HTTP registry URLs, credentials in git URLs, unpinned git dependencies, auditwheel skips, sensitive file includes, unsafe RUSTFLAGS, and curl-pipe-to-shell in build hooks
