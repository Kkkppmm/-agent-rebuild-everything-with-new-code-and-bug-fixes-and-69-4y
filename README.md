...
- **SecurityScanner** — Unified static security scan combining secrets, injections, dangerous calls, insecure random, weak crypto, log injection, SSRF, path traversal, and 73 security checks
...
- **InsecureGraphqlSettingsAnalyzer** — Detect introspection, GraphiQL/playground, empty GRAPHENE middleware, and public views
- **InsecureWebhookSettingsAnalyzer** — Detect hardcoded webhook secrets, disabled signature verification, HTTP URLs, and csrf_exempt handlers
- **InsecureJwtSettingsAnalyzer** — Detect hardcoded JWT secrets, 'none' algorithm, disabled verification, long expiry, and tokens in query strings
