...
- **SvelteKitAnalyzer** — Audit `svelte.config.*` for disabled CSRF checks, exposed dev servers, disabled CSP, internal proxy targets, and adapter credential leaks
- **GatsbyAnalyzer** — Audit `gatsby-config.*` for siteMetadata secrets, internal proxy targets, disabled GraphQL IDE protections, and plugin option secrets
- **QwikAnalyzer** — Audit `qwik.config.*` and `vite.config.*` for exposed dev/preview servers, disabled origin checks, and adapter credential leaks
