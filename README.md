...
- **PrecommitAnalyzer** — Audit pre-commit config for unpinned hook revisions, local hooks, unsafe bash entries, and secrets in config
- **MakefileAnalyzer** — Audit Makefiles for destructive rm commands, sudo usage, chmod 777, curl-pipe-to-shell, and hardcoded secrets
- **KubernetesAnalyzer** — Audit K8s manifests for privileged mode, host namespaces, secrets in env, :latest tags, and missing resource limits
